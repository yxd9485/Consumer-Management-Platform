    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
            	action="<%=cpath %>/stat/doStatInfoAdd.do">
							...
							<tr>
								...
								<td class="ab_main" colspan="3">
									<select name="statType" class="form-control input-width-larger search" autocomplete="off" >
										<option value="SG00001">经典箱啤500*6</option>
										<option value="SG00002">经典箱啤500*12</option>
										<option value="SG00003">经典罐啤500*12</option>
										<option value="SG00004">纯生箱啤</option>
										<option value="SG00005">纯生罐啤</option>
										<option value="SG00006">崂山箱+罐啤</option>
										<option value="SG00008">青啤项目月报</option>
										<option value="SG00009">青啤总部月报</option>
										<option value="SG00007">其他</option>
									</select>
										<label class="validate_tips"></label>
								</td>
							</tr>
							<tr>
							  ...
							  <tr>
							         <td class="ab_left"><label class="title">省区：<span class="required">*</span></label></td>
							          ...
							          <option value="${server.projectServerName}">${server.serverName}</option>
							           </c:forEach>
							     </c:if>
							         </select>
							        <label class="validate_tips"></label>
											...
											<c:if test="${!empty skuList}">
												<c:forEach items="${skuList}" var="sku">
													<div class="${sku.projectServerName} dasha" style="display: none">
														<label id="skuKeyCheckBoxLabel"><input class="wtf" name="skuKey" id="skuKeyCheckBox" type="checkbox" value="${sku.projectServerName}/${sku.serverName}/${sku.skuKey}/${sku.skuName}" tag="validate"/>${sku.serverName}-${sku.skuName}-<span style="color: red;">${sku.statName}</span></label>
													</div>
												</c:forEach>
											</c:if>
										</div>
