    ...
    <script>
        $(function(){
            ...
            $("#addItem").click(function(){
                var url = "<%=cpath%>/mnRank/showRankAdd.do";
            ...
            $("a.edit").on("click", function(){
                ...
                var url = "<%=cpath%>/mnRank/showRankEdit.do?infoKey="+key;
                $("form").attr("action", url);
                $("form").submit();
            });
            ...
            $("a.view").on("click", function(){
                var key = $(this).parents("td").data("key");
                var url = "<%=cpath%>/mnRank/showRankEdit.do?infoKey="+key+"&tabFlag=1";
                $("form").attr("action", url);
                $("form").submit();
            });
            // 导出
            ...
            $("a.export").on("click", function(){
                var key = $(this).parents("td").data("key");
                var url = "<%=cpath%>/mnRank/rankExport.do?infoKey=" + key;
                $("form").attr("action", url);
                $("form").submit();
                var url = "<%=cpath%>/mnRank/showRankList.do?";
                $("form").attr("action", url);
            });
            ...
            $("a.del").on("click", function(){
                var key = $(this).parents("td").data("key");
                var url = "<%=cpath%>/mnRank/deleteRank.do?infoKey="+key;
    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <div class="widget box">
                <div class="row">
                    <div class="col-md-2"><span style="font-size: 15px; color: white; font-weight: bold;">排行榜活动查询</span></div>
                ...
                <div class="widget-content form-inline">
                    <form class="listForm" method="post"
                          action="<%=cpath%>/mnRank/showRankList.do">
            ...
            <div class="widget box" id="tab_1_1">
                <div class="row">
                    <div class="col-md-2"><span style="font-size: 15px; color: white; font-weight: bold;">积盖活动列表</span></div>
                </div>
                    ...
                    <div role="grid" class="dataTables_wrapper form-inline" id="DataTables_Table_0_wrapper">
                        <table class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10"
                            ...
                            <tr>
                                ...
                                <th style="width:9%;">活动名称</th>
                                <th style="width:10%;">活动有效期</th>
                                <th style="width:6%;">活动状态</th>
                                <th style="width:6%;">活动类型</th>
                                    ...
                                    <c:forEach items="${resultList}" var="item" varStatus="idx">
                                        <tr align="center">
                                            ...
                                            <td style="text-align:center;">
					        			     <span>
										     	<c:choose>
                                                    <c:when test="${item.activityType eq '0'}">组织架构</c:when>
                                                    <c:when test="${item.activityType eq '1'}">行政区域</c:when>
                                                </c:choose>
										     </span>
                                            </td>
                                            <td>
                                            ...
                                            <td data-key="${item.infoKey}" style="text-align: center;">
                                                ...
                                                <a class="btn btn-xs view btn-green"><i class="iconfont icon-peizhi"></i>&nbsp;查看</a>
                                                <c:if test="${item.isBegin ne '0'}">
                                                    <a class="btn btn-xs export btn-blue"><i class="iconfont icon-xiazai"></i>&nbsp;导出</a>
                                                </c:if>
                                                <c:if test="${item.isBegin eq '0'}">
                                                    <a class="btn btn-xs del btn-red"><i class="iconfont icon-yichu"></i>&nbsp;删 除</a>
                                                </c:if>
                                            </td>
                                        </tr>
                                    </c:forEach>
