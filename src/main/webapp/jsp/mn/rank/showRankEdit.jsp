...
<head>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            //根据活动有效状态判断 字段是否可以修改
            if(${isBegin}){
                // $("#divSku input").attr("disabled",true);
                $("#divActivityType input").attr("disabled",true);
                $("#divRole input").attr("disabled",true);
                $("#divCascade input").attr("disabled",true);
                $("#startTime").attr("disabled",true);
            }
            $(".btnReturn").click(function(){
                var url = "<%=cpath%>/mnRank/showRankList.do" + "?vjfSessionId=" + $("#vjfSessionId").val();
        ...
        function validForm() {
           ...
           // 图片上传验证
            ...
            if(!activityAreaCode && !department && !${isBegin}){
                $.fn.alert("活动区域必需选择");
                return false;
            }
            //富文本
        ...
        function initPage() {
            ...
            setTimeout(function () {
                $("input[name='activityRole']").triggerHandler("change")
            },500)

            $("input[name='activityType']").on("change", function (val) {
                if ($("input[name='activityType']:checked").val()== '1'){
                    departmentTreeObj.hide();
                    $("#mndepartmentTree").hide();
                    $("#mndepartmentTree4_5").hide();
                    $("#areaTree").show();
                } else {
    ...
    <div class="row mart20">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
                  action="<%=cpath %>/mnRank/rankUpdate.do">
                <input type="hidden" name="roleDescribe" id="roleDescribe" value="">
                <input type="hidden" name="initBannerUrl" id="initBannerUrl" value="${activityCog.initBannerUrl}">
                <input type="hidden" name="activityBannerUrl" id="activityBannerUrl" value="${activityCog.activityBannerUrl}">
                    ...
                    <div class="widget-content panel no-padding">
                        <table class="active_board_table">
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="startTime" id="startTime" value="${activityCog.startTime}"  class="form-control  input-width-larger Wdate required"
                                               tag="validate" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'%y-%M-%d %H:%m:%s', maxDate:'#F{$dp.$D(\'endTime\')}'})" />
                                        <span class="blocker en-larger">--</span>
                                        <input name="endTime" id="endTime" value="${activityCog.endTime}" class="form-control input-width-larger Wdate required  "
                                               tag="validate" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss', minDate:'#F{$dp.$D(\'startTime\')}'}) " />
                                        <label class="validate_tips"></label>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content" id="divRole">
                                        <input type="radio" class="tab-radio required" <c:if test="${activityCog.activityRole eq '0'}">checked</c:if> name="activityRole"  tag="validate" value="0" style="float:left; cursor: pointer;" />
                                        <span class="blocker en-larger">配送员</span>
                                        <input type="radio" class="tab-radio required" <c:if test="${activityCog.activityRole eq '3'}">checked</c:if> name="activityRole"  tag="validate" value="3" style="float:left; cursor: pointer;" />
                                        <span class="blocker en-larger">分销商</span>
                                        <input type="radio" class="tab-radio required" <c:if test="${activityCog.activityRole eq '1'}">checked</c:if> name="activityRole"  tag="validate" value="1" style="float:left; cursor: pointer;" />
                            ...
                            <tr id="sku">
                                ...
                                <td class="ab_main" colspan="3"  id="divSku">
                                        ...
                                        <c:if test="${!empty skuList}">
                                            <c:forEach items="${skuList}" var="item" varStatus="idx">
                                                <div style="float: left;position: relative;width: 20%" class="srearchSku"  sku="${item.skuName}" >
                                                    <input type="checkbox" name="skuKeys" <c:if test="${fn:contains(activityCog.skuKeys,item.skuKey)}">checked</c:if> sku="${item.skuName}" class="tab-radio  srearchSku"  value="${item.skuKey}">
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3"  id="divCascade">
                                    <div class="content">
                                        <input type="radio" class="tab-radio required"  tag="validate"  <c:if test="${activityCog.cascadeFlag eq '1'}">checked</c:if> name="cascadeFlag" value="1" style="float:left; cursor: pointer;" />
                                        <span class="blocker en-larger">包含</span>
                                        <input type="radio" class="tab-radio required"  tag="validate" <c:if test="${activityCog.cascadeFlag eq '0'}">checked</c:if> name="cascadeFlag" value="0" style="float:left; cursor: pointer;" />
                    ...
                    <div class="widget-content panel no-padding">
                        <table class="active_board_table">
                            <tr>
                                ...
                                <td class="ab_main" colspan="3"  id="divActivityType">
                                    <div class="content">
                                        <input type="radio" class="tab-radio required"   tag="validate" <c:if test="${activityCog.activityType eq '0'}">checked</c:if>  name="activityType" value="0" style="float:left; cursor: pointer;" />
                                        <span class="blocker en-larger">组织架构</span>
                                        <input type="radio" class="tab-radio required"  tag="validate"  <c:if test="${activityCog.activityType eq '1'}">checked</c:if>  name="activityType" value="1" style="float:left; cursor: pointer;" />
                                        <span class="blocker en-larger">行政区域</span>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            <tr id="areaTree" <c:if test="${activityCog.activityType eq '0'}"> hidden</c:if>>
...
<script onload="this" >
    ...
    function addUrlImg(view_DOM,imgUrl,md5) {
        var html = '<li data-md5="'+md5+'">'+
            ...
            '<span class="icon-success" title="正在上传..."></span>'+
    ...
    var a = $("#upload_duixiang").FraUpload({
        ...
        onLoad: function (e,view) { // 选择文件的回调方法
            let goodsUrl = 	$("#initBannerUrl").val()
            var goodsUrls = goodsUrl.split(",");
            for (let i = 0; i < goodsUrls.length; i++) {
                let md5 = goodsUrls[i];
                if(md5){
                    addUrlImg(view,goodsUrls[i],md5)
