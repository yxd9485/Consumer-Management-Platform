    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
            	action="<%=cpath %>/adPub/adShopEdit.do">
                		...
                		<table class="active_board_table">
                			<tr style="display:none">
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name=adShopKey tag="validate"
	                       					class="form-control required" autocomplete="off" maxlength="30" value = "${vpsAdPub.adShopKey}" />
	                       			</div>
	                       		</td>
	                       	</tr>
							<tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="title" tag="validate"
	                       					class="form-control required" autocomplete="off" maxlength="30" value = "${vpsAdPub.title}" <c:if test= "${vpsAdPub.isDefault eq '0'}"> readonly ="readonly" </c:if>
							...
							<tr>
                                ...
                                <td class="ab_main area" colspan="3">
                                     <div class="content area" style="display: flex; margin: 5px 0px;">
                                         <select name="provinceAry" class="zProvince form-control input-width-normal required" tag="validate"  <c:if test="${vpsAdPub.isDefault eq '0'}"> readonly ="readonly" </c:if>></select>
                                         <select name="cityAry" class="zCity form-control input-width-normal required" <c:if test="${vpsAdPub.isDefault eq '0'}"> readonly ="readonly" </c:if>></select>
                                         <select name="countyAry" class="zDistrict form-control input-width-normal required" <c:if test="${vpsAdPub.isDefault eq '0'}"> readonly ="readonly" </c:if>></select>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
                                        ...
                                        <input name="stGmt" id="startDate" class="form-control input-width-medium Wdate required preTime" value="${fn:substring(vpsAdPub.stGmt, 0, 19)}"
                                            tag="validate" autocomplete="off" style="width: 180px !important;" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss', minDate:'%y-%M-%d'})" <c:if test="${vpsAdPub.isDefault eq '0'}"> readonly ="readonly" </c:if>/>
                                        ...
                                        <input name="endGmt" id="endDate" class="form-control input-width-medium Wdate required sufTime" value="${fn:substring(vpsAdPub.endGmt, 0, 19)}"
                                            tag="validate" style="width: 180px !important;" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss', minDate:'%y-%M-%d'})" <c:if test="${vpsAdPub.isDefault eq '0'}"> readonly ="readonly" </c:if>/>
                                       	<label class="validate_tips"></label>
	                       			</div>
	                       		</td>
	                       	</tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="sequenceNo" tag="validate"
	                       					class="form-control required" autocomplete="off" maxlength="2" value = "${vpsAdPub.sequenceNo}" style="width: 120px;"/>
	                       				<label class="validate_tips"></label>
	                       			</div>
	                       		</td>
	                       	</tr>
                           ...
                           <tr <c:if test= "${vpsAdPub.isDefault eq '0'}"> STYLE="display: none" </c:if>>
							...
							<tr>
								<td class="ab_left"><label class="title">选择素材：<span class="required">*</span>
									<c:if test="${fn:startsWith(currentUser.projectServerName, 'mengniu')}"><br/><span class="blocker en-larger" style="color: #b81900">尺寸建议702*373px</span></c:if>
								</label></td>
								<td class="ab_main" colspan="3">
									<div class="content">
										<div class="item">
											<div class="item">
												<a href="<%=cpath %>/picLib/picLibList.do?vjfSessionId=${vjfSessionId}&group=10"
    ...
    opacity: 0;"></a>
                	...
                	<div class="active_table_submit mart20">
			            <div class="button_place">
					    	...
					    	<button class="btn btnReturn btn-radius3"  data-url="<%=cpath%>/adPub/showShopAdList.do">返 回</button>
			            </div>
	            	</div>
            	</div>
            </form>
