...
<head>
    ...
    <link type="text/css" rel="stylesheet"
          href="<%=cpath %>/inc/upload/jquery.imgareaselect-0.9.10/uploadify/css/uploadify.css"/>
    <script type="text/javascript"
            src="<%=cpath %>/inc/upload/jquery.imgareaselect-0.9.10/uploadify/widgets/swfobject.js"></script>
    <script type="text/javascript"
            src="<%=cpath %>/inc/upload/jquery.imgareaselect-0.9.10/uploadify/widgets/jquery.uploadify.v2.1.0.min.js"></script>
    ...
    <script>
        ...
        $(function () {
            ...
            updateVideo();
            // 初始化筛选区域
            var filterAreaCodeAry = "${vpsActVideo.areaCode}".split(",");
            ...
            var imgData = '${vpsActVideo.videoCoverImage}';
            if (imgData) {
                showImg(imgData);
            }
        });
    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
                  action="<%=cpath %>/actVideo/update.do">
                <input type="hidden" name="videoKey" value="${vpsActVideo.videoKey}">
                <input type="hidden" name="videoCoverImage" value="${vpsActVideo.videoCoverImage}">
                <div class="widget box">
                    <div class="widget-header">
                        <h4><i class="iconfont icon-xinxi"></i>修改视频</h4>
                    </div>
                        ...
                        <table class="active_board_table">
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="title" tag="validate" value="${vpsActVideo.title}"
                            ...
                            <tr>
                                ...
                                <td class="ab_left"><label class="title">展示区域：<span class="required">*</span></label> </td>
                            ...
                            <tr>
                                <td class="ab_left"><label class="title">精准触达人群：<span class="white">*</span></label>
                                </td>
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <label class="blocker en-larger" style="margin-left: 2px;">
                                            <input name="crowdLimitType" type="radio" value='0'
                                                <c:if test="${vpsActVideo.crowdLimitType eq '0'}">checked</c:if>/>不限制</label>
                                        <label class="blocker en-larger" style="margin-left: 2px;">
                                            <input name="crowdLimitType" type="radio" value='1'
                                                <c:if test="${vpsActVideo.crowdLimitType eq '1'}">checked</c:if>/>黑名单用户不可领取</label>
                                        <c:if test="${not empty(groupList)}">
                                            <label class="blocker en-larger" style="margin-left: 2px;">
                                                <input name="crowdLimitType" type="radio" value='2'
                                                    <c:if test="${vpsActVideo.crowdLimitType eq '2'}">checked</c:if>/>指定群组参与</label>
                                            <label class="blocker en-larger" style="margin-left: 2px;">
                                                <input name="crowdLimitType" type="radio" value='3'
                                                    <c:if test="${vpsActVideo.crowdLimitType eq '3'}">checked</c:if>/>指定群组不可参与</label>
                                        </c:if>
                                    </div>
                                </td>
                            </tr>
                            <c:if test="${not empty(groupList)}">
                                <tr id="userGroupTr" <c:if
                                        test="${vpsActVideo.crowdLimitType eq '0' or vpsActVideo.crowdLimitType eq '1'}"> style="display: none;" </c:if>>
                                    <td class="ab_left"><label class="title"></label></td>
                                    <td class="ab_main" colspan="3">
                                        <div>
                                            <c:forEach items="${groupList }" var="group">
                                                <c:set var="checkedStr" value=""></c:set>
                                                <c:forEach items="${fn:split(vpsActVideo.userGroupIds, ',')}" var='groupId'>
                                                    <c:if test="${groupId eq group.id}">
                                                        <c:set var="checkedStr" value="checked='checked'"></c:set>
                                                    </c:if>
                                                </c:forEach>
                                                <label class="blocker en-larger">
                                                    <input name="userGroupIds"
                                                           type="checkbox"
                                                           value='${group.id }' ${checkedStr}/>${group.name}
                                                </label>
                                            </c:forEach>
                                        </div>
                                    </td>
                            ...
                            <tr class="jumpurl">
                                <td class="ab_left"><label class="title">关联商品：</label> </td>
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <select name="goodsId" class="form-control  input-width-larger ">
                                            ...
                                            <c:forEach items="${goodsList}" var="goods" varStatus="st">
                                                ...
                                                <c:if test="${vpsActVideo.goodsId eq goods.goodsId}">selected</c:if>>${goods.goodsName}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td class="ab_left"><label class="title">选择素材：</label>
                                </td>
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="hidden" name="videoPath" id="pathVideo" value="${vpsActVideo.videoPath}"/>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    ...
                                    <video controls="controls" id="video">
                                        <source src="<%=filePath%>/${vpsActVideo.videoPath}" id="source"
                                                type="video/mp4">
                                        你的浏览器不支持播放视频
                                    </video>
                                </td>
                    ...
                    <div class="active_table_submit mart20">
                        <div class="button_place">
                            ...
                            <button class="btn btnReturn btn-radius3" data-url="<%=cpath%>/actVideo/showVideoList.do">返
                                回
                            </button>
                        </div>
                    </div>
