...
<html>
  <head>
    ...
    var customerDefaults = {
		...
		function initPage() {
			$("#defaultUrl").attr("src",src="https://img.vjifen.com/images/vma/"+'${vpsActRule.defaultUrl}');
			$("input[name= defaultUrl]").val('${vpsActRule.defaultUrl}');
			$("input[name= defaultPicWidth]").val(${vpsActRule.defaultPicWidth});
			$("input[name= defaultPicHeight]").val(${vpsActRule.defaultPicHeight});
			$("input[name= defaultPicX]").val(${vpsActRule.defaultPicX});
			$("input[name= defaultPicY]").val(${vpsActRule.defaultPicY});
			if("${vpsActRule.limitUrl}"!=null && "${vpsActRule.limitUrl}" !="" && "${vpsActRule.limitUrl}" !='undefined'){
				$("#limitUrl").attr("src",src="https://img.vjifen.com/images/vma/"+'${vpsActRule.limitUrl}');
				$("input[name= limitUrl]").val('${vpsActRule.limitUrl}');
				$("input[name= limitPicWidth]").val(${vpsActRule.limitPicWidth});
				$("input[name= limitPicHeight]").val(${vpsActRule.limitPicHeight});
				$("input[name= limitPicX]").val(${vpsActRule.limitPicX});
				$("input[name= limitPicY]").val(${vpsActRule.limitPicY});
			}
            // 返回
    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
            	action="<%=cpath %>/actRule/actRuleEdit.do">
<%--            	<input type="hidden" name="defaultUrl" id="defaultUrl" value="">--%>
<%--            	<input type="hidden" name="limitUrl" id="limitUrl" value="">--%>
            	<div class="widget box">
            		<div class="widget-header">
            			<h4><i class="iconfont icon-xinxi"></i>修改活动规则</h4>
            		</div>
                		...
                		<table class="active_board_table">
                			<tr style="display:none">
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name=actRuleKey tag="validate"
	                       					class="form-control required" autocomplete="off" maxlength="30" value = "${vpsActRule.actRuleKey}" />
	                       			</div>
	                       		</td>
	                       	</tr>
							<tr style="display:none">
								<td class="ab_main" colspan="3">
									<div class="content">
										<input name="vcodeActivityKey" tag="validate"
											   class="form-control required" autocomplete="off" maxlength="30" value = "${vpsActRule.vcodeActivityKey}" />
									</div>
								</td>
							</tr>
                			<tr>
                                <td class="ab_left"><label class="title">扫码活动：<span class="required">*</span></label></td>
                                <td class="ab_main sku" colspan="3">
                                    <div class="content sku promotionSku" style="display: flex; margin: 5px 0px;">
                                        <input class="form-control input-width-larger required" id="skuNm" name="skuNm" tag="validate" value ="${vpsActRule.vcodeActivityName}" readonly ="readonly" />
									...
									<div class="content">
										<div class="item">
											...
											<input name="defaultPicWidth" tag="validate"
												   class="form-control required" autocomplete="off" maxlength="10" placeholder = "宽"  type="hidden" value = "${vpsActRule.defaultPicWidth}"/>
											<input name="defaultPicHeight" tag="validate"
												   class="form-control required" autocomplete="off" maxlength="10"  placeholder = "高" type="hidden" value = "${vpsActRule.defaultPicHeight}"/>
											<input name="defaultPicX" tag="validate"
												   class="form-control required" autocomplete="off" maxlength="10"  placeholder = "坐标x"type="hidden" value = "${vpsActRule.defaultPicX}"/>
											<input name="defaultPicY" tag="validate"
												   class="form-control required" autocomplete="off" maxlength="10" placeholder = "坐标y" type="hidden" value = "${vpsActRule.defaultPicY}"/>
										</div>
                			...
                			<tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
                                        ...
                                        <input name="limitStGmt" id="limitStGmt" class="form-control input-width-medium Wdate required preTime"  value="${fn:substring(vpsActRule.limitStGmt, 0, 19)}"
                                        ...
                                        <input name="limitEndGmt" id="limitEndGmt" class="form-control input-width-medium Wdate required sufTime" value="${fn:substring(vpsActRule.limitEndGmt, 0, 19)}"
								...
								<td class="ab_main" colspan="3">
									<div class="content">
										<div class="item">
											...
											<input type="hidden" name="limitUrl" value = "${vpsActRule.limitUrl}">
											<input name="limitPicWidth" tag="validate"
												   class="form-control required" autocomplete="off" maxlength="10" placeholder = "宽"  type="hidden" value = "${vpsActRule.limitPicWidth}"/>
											<input name="limitPicHeight" tag="validate"
												   class="form-control required" autocomplete="off" maxlength="10"  placeholder = "高" type="hidden" value = "${vpsActRule.limitPicHeight}"/>
											<input name="limitPicX" tag="validate"
												   class="form-control required" autocomplete="off" maxlength="10"  placeholder = "坐标x"type="hidden" value = "${vpsActRule.limitPicX}"/>
											<input name="limitPicY" tag="validate"
												   class="form-control required" autocomplete="off" maxlength="10" placeholder = "坐标y" type="hidden" value = "${vpsActRule.limitPicY}"/>
										</div>
										<div style="clear: left;"></div>
									</div>
								</td>
