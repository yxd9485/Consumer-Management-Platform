...
<html>
  ...
  <body>
	<form class="listForm" method="post" action="<%=cpath%>/vpointsExchange/showExchangeExpressList.do">
		  ...
		  <input type="hidden" name="seckillActivityKey" value="${seckillActivityKey }">
		  <input type="hidden" name="tabsFlag" value="3">
	</form>
     <div class="col-md-12 tabbable tabbable-custom">
         <div class="widget box" id="tab_1_1">
             <div class="row">
                <div class="col-md-2"><span style="font-size: 15px; font-weight: bold;">秒杀成交数据</span></div>
...
<!--                                 待发货 -->
                            <tr>
                                ...
                                <th style="width:11%;" data-ordercol="e.user_name">订单类型</th>
                                ...
                                <th style="width:6%;" data-ordercol="e.express_status">订单状态</th>
                            </tr>
		        		</thead>
		        <tbody>
		        	<c:choose>
		        	...
		        	<tr>
                        ...
                        <td>
                            <span >
                                    <c:choose>
                                        <c:when test="${item.orderType eq '0'}">常规订单</c:when>
                                        <c:when test="${item.orderType eq '1'}">秒杀订单</c:when>
                                        <c:when test="${item.orderType eq '2'}">拼团订单</c:when>
                                        <c:when test="${item.orderType eq '3'}">买赠订单</c:when>
                                        <c:when test="${item.orderType eq '4'}">礼品卡订单</c:when>
                                        <c:when test="${item.orderType eq '5'}">组合优惠订单</c:when>
                                        <c:when test="${item.orderType eq '6'}">折扣订单</c:when>
                                        <c:when test="${item.orderType eq '7'}">换购订单</c:when>
                                    </c:choose>
                            </span>
                        </td>
                        <td style="text-align:center;">
