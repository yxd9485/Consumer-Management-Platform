	...
	<script>
		$(function(){
			...
			$("#addItem").click(function(){
				var url = "<%=cpath%>/seckillActivity/showSeckillActivityAdd.do";
			...
			$("a.edit").on("click", function(){
				...
				var url = "<%=cpath%>/seckillActivity/showSeckillActivityEdit.do?infoKey="+key;
				$("form").attr("action", url);
				$("form").submit(); 
			});
			// 数据
			...
			$("a.data").on("click", function(){
				var key = $(this).parents("td").data("key");
				var url = "<%=cpath%>/seckillActivity/showSeckillActivityData.do?infoKey="+key;
				$("form").attr("action", url);
				$("form").submit(); 
			});
			...
			$("a.stop").on("click", function(){
				var key = $(this).parents("td").data("key");
				var url = "<%=cpath%>/seckillActivity/updateActivityStop.do";
				$.fn.confirm("停用后将不可恢复，确定停用该秒杀活动？", function() {
					$.ajax({
	                    type: "POST",
	                    url: url,
	                    data: {"infoKey": key},
			...
			$("a.del").on("click", function(){
				var key = $(this).parents("td").data("key");
				var url = "<%=cpath%>/seckillActivity/delActivity.do";
  ...
  <body>
    ...
    <div class="row">
        <div class="col-md-12 tabbable">
            <a href="<%=cpath%>/seckillActivity/showSeckillActivityList.do?vjfSessionId=${vjfSessionId}&tabsFlag=1" 
                 class="btn <c:if test="${tabsFlag eq '1'}">btn-blue</c:if>">待上线</a>
            <a href="<%=cpath%>/seckillActivity/showSeckillActivityList.do?vjfSessionId=${vjfSessionId}&tabsFlag=2" 
                 class="btn <c:if test="${tabsFlag eq '2'}">btn-blue</c:if>">已上线</a>
            <a href="<%=cpath%>/seckillActivity/showSeckillActivityList.do?vjfSessionId=${vjfSessionId}&tabsFlag=3" 
                 class="btn <c:if test="${tabsFlag eq '3'}">btn-blue</c:if>">已下线</a>
            <a href="<%=cpath%>/seckillActivity/showSeckillActivityList.do?vjfSessionId=${vjfSessionId}&tabsFlag=4" 
            ...
            <div class="widget box">
	            ...
	            <div class="widget-content form-inline">
		            <form class="listForm" method="post"
	                    action="<%=cpath%>/seckillActivity/showSeckillActivityList.do">
                        ...
                        <table class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10"
                            ...
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <th style="width:8%;" data-ordercol="a.activity_no">秒杀编号</th>
                                    <th style="width:15%;" data-ordercol="a.activity_name">活动名称</th>
                                    <th style="width:15%;" data-ordercol="g.goods_name">秒杀商品</th>
                                    <th style="width:6%;" data-ordercol="a.seckill_pay" data-orderdef>秒杀价格</th>
                                    <th style="width:5%;" data-ordercol="a.seckill_Vpoints" data-orderdef>秒杀积分</th>
                                    <th style="width:5%;" data-ordercol="a.is_stop" data-orderdef>是否停用</th>
                                    <th style="width:10%;" data-ordercol="a.begin_date" data-orderdef>活动时间</th>
                                    <th style="width:7%;" data-ordercol="a.seckill_periods_num" data-orderdef>每期秒杀库存</th>
					        ...
					        <tbody>
					        	...
					        	<tr>
					        		...
					        		<td>
					        			<span>${item.seckillPay}</span>
					        		</td>
					        		<td>
					        			<span>${item.seckillVpoints}</span>
					        		</td>
					        		<td style="text-align:center;">
					        			<c:choose>
					        		...
					        		<td>
                                        ...
                                        <hr style="margin-bottom: 4px; margin-top: 0px; border-top: 0px solid white;">
                                        <span>每天${item.beginTime}到${item.endTime}</span>
					        		</td>
					        		<td>
					        			<span>初始库存：${item.seckillPeriodsNum}</span>
					        			<hr style="margin-bottom: 4px; margin-top: 0px; border-top: 1px solid gray;">
					        			<span>剩余库存：${item.seckillPeriodsRemainsNum}</span>
					        		</td>
					        		<td>
					        			<span>${item.createTime}</span>
					        		</td>
                                    <td data-key="${item.infoKey}" style="text-align: center;">
                                    	<a class="btn btn-xs data btn-blue"><i class="iconfont icon-xinxi"></i>&nbsp;数 据</a>
                                        <c:choose>
