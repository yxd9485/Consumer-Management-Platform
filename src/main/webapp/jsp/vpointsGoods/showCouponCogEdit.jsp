  ...
  <head>
	...
	<script>
		...
		function initPage() {
            ...
            $("[name='couponPayType']:checked").change();
            $("[name='moneyFullLimit']").val("${couponCog.moneyFullLimit}");
            $("[name='vpointsFullLimit']").val("${couponCog.vpointsFullLimit}");
            $("[name='moneyFullReduction']").val("${couponCog.moneyFullReduction}");
            $("[name='vpointsFullReduction']").val("${couponCog.vpointsFullReduction}");
            $("[name='moneyDirectReduction']").val("${couponCog.moneyDirectReduction}");
            $("[name='vpointsDirectReduction']").val("${couponCog.vpointsDirectReduction}");
            $("[name='couponDiscount']").val("${couponCog.couponDiscount}");
            $("[name='discountMaxMoney']").val("${couponCog.discountMaxMoney}");
            $("[name='discountMaxVpoints']").val("${couponCog.discountMaxVpoints}");
            ...
            $("[name='couponGoodsType']").on("change", function(){
                ...
                if (couponGoodsType == "1") {
                    ...
                    $("div.goods:gt(0)").closest("div").remove();
                } else {
            ...
            $("form").on("click", "#addGoods", function() {
                ...
                if ($(this).text() == '新增') {
                    ...
                    $copySku.find("[name='goodsIdAry'] option").eq(0).attr("selected", "selected");
            ...
            $("[name='expireDateType']:checked").change();
            

            // 初始化图片显示
            var imgData='${couponCog.couponActivityImgUrl}';
            if(imgData){
                showImg(imgData);
            }
		}
    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
            	action="<%=cpath %>/coupon/doCouponCogEdit.do">
            	<input type="hidden" name="tabsFlag" value="${tabsFlag}"/>
            	<input type="hidden" name="couponKey" value="${couponCog.couponKey}"/>
                <input type="hidden" name="couponActivityImgUrl" value="${couponCog.couponActivityImgUrl}"/>
                		...
                		<table class="active_board_table">
                			<tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="couponName" tag="validate" value="${couponCog.couponName}"
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" class="tab-radio serverdis" name="couponPayType" value="0" data-serverdis="henanpz" style="float:left;" <c:if test="${couponCog.couponPayType eq '0'}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">积分</span>
                                        <input type="radio" class="tab-radio" name="couponPayType" value="1" style="float:left;" <c:if test="${couponCog.couponPayType eq '1'}">checked</c:if>/>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <select name="couponType" class="form-control input-width-larger">
                                            <option value="0" <c:if test="${couponCog.couponType eq '0'}">selected</c:if>>满减券</option>
                                            <option value="1" <c:if test="${couponCog.couponType eq '1'}">selected</c:if>>直减券</option>
                                            <option value="2" <c:if test="${couponCog.couponType eq '2'}">selected</c:if>>折扣券</option>
                                        </select>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" class="tab-radio" name="couponGoodsType" value="0" style="float:left;" <c:if test="${couponCog.couponGoodsType eq '0'}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">通用券（全部商品）</span>
                                        <input type="radio" class="tab-radio" name="couponGoodsType" value="1" style="float:left;" <c:if test="${couponCog.couponGoodsType eq '1'}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">单品券</span>
                                        <input type="radio" class="tab-radio" name="couponGoodsType" value="2" style="float:left;" <c:if test="${couponCog.couponGoodsType eq '2'}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">指定商品可用券</span>
                                        <input type="radio" class="tab-radio" name="couponGoodsType" value="3" style="float:left;" <c:if test="${couponCog.couponGoodsType eq '3'}">checked</c:if>/>
                            ...
                            <tr class="goodsLimit">
                                ...
                                <td class="ab_main prize" colspan="3">
                                    <c:choose>
                                        <c:when test="${empty couponCog.goodsIdAry}">
	                                        <div class="content goods" style="display: flex; margin: 5px 0px;">
                                        ...
                                        <c:otherwise>
		                                    <c:forEach items="${couponCog.goodsIdAry}" var="goodsId" varStatus="idx">
		                                        <div class="content goods" style="display: flex; margin: 5px 0px;">
		                                            <select class="form-control required" name="goodsIdAry" tag="validate" disabled="disabled" style="width: 450px !important;">
		                                                <option value="">请选择</option>
			                                            <c:if test="${!empty goodsLst}">
			                                            <c:forEach items="${goodsLst}" var="goods">
			                                            <option value="${goods.goodsId}"  <c:if test="${goodsId eq goods.goodsId}">selected</c:if>>${goods.goodsName}</option>
			                                            </c:forEach>
			                                            </c:if>
		                                            </select>
		                                            <label class="title mart5 btn-txt-add-red" style="font-weight: normal; margin-left: 5px;" id="addGoods">${idx.count == 1 ? '新增' : '删除'}</label>
		                                            <label class="validate_tips"></label>
		                                        </div>
		                                    </c:forEach>
                                        </c:otherwise>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <select name="couponChannel" class="form-control required input-width-larger"  tag="validate">
                                            <option value="">请选择</option>
                                            <option value="0" <c:if test="${couponCog.couponChannel eq '0'}">selected</c:if>>商城领取 （支持商城领取及扫码发券）</option>
                                            <option value="1" <c:if test="${couponCog.couponChannel eq '1'}">selected</c:if>>扫码发券</option>
                                        </select>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="couponNum" tag="validate" value="${couponCog.couponNum}"
                			...
                			<tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
                                        ...
                                        <input name="receiveStartDate" id="receiveStartDate" class="form-control input-width-medium Wdate required preTime"  value="${couponCog.receiveStartDate}"
                                            tag="validate" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', maxDate:'#F{$dp.$D(\'receiveEndDate\')}'})"  />
                                        <span class="blocker en-larger">至</span>
                                        <input name="receiveEndDate" id="receiveEndDate" class="form-control input-width-medium Wdate required sufTime" value="${couponCog.receiveEndDate}"
                			...
                			<tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<span class="blocker">从</span>
                                        <input name="startDate" id="startDate" class="form-control input-width-medium Wdate required preTime"  value="${couponCog.startDate}"
                                            tag="validate" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', maxDate:'#F{$dp.$D(\'endDate\')}'})"  />
                                        <span class="blocker en-larger">至</span>
                                        <input name="endDate" id="endDate" class="form-control input-width-medium Wdate required sufTime" value="${couponCog.endDate}"
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" class="tab-radio" name="expireDateType" value="0" style="float:left;" <c:if test="${couponCog.expireDateType eq '0'}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">指定日期</span>
                                        <input name="expireDateLimit" class="form-control input-width-medium Wdate required" value="${couponCog.expireDateLimit}"
                                            tag="validate" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:'%y-%M-%d'})" />
                                        <input type="radio" class="tab-radio" name="expireDateType" value="1" style="float:left; margin-left: 10px !important;" <c:if test="${couponCog.expireDateType eq '1'}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">有效天数</span>
                                        <input name="expireDateDays" tag="validate" disabled="disabled" value="${couponCog.expireDateDays}"
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <textarea name="couponDesc" rows="5" tag="validate"
                                            class="form-control" autocomplete="off" maxlength="200" >${couponCog.couponDesc}</textarea>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                	...
                	<div class="widget-content panel no-padding filteruser">
                		<table class="active_board_table">
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        ...
                                        <input name="personDayLimit" value="${couponCog.personDayLimit}" class="form-control input-width-small number positive maxValue" tag="validate" maxVal="99999" autocomplete="off" />
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        ...
                                        <input name="personWeekLimit" value="${couponCog.personWeekLimit}" class="form-control input-width-small number positive maxValue" tag="validate" maxVal="99999" autocomplete="off" />
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        ...
                                        <input name="personMonthLimit" value="${couponCog.personMonthLimit}" class="form-control input-width-small number positive maxValue" tag="validate" maxVal="99999" autocomplete="off" />
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        ...
                                        <input name="personTotalLimit" value="${couponCog.personTotalLimit}" class="form-control input-width-small number positive maxValue" tag="validate" maxVal="99999" autocomplete="off" />
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" class="tab-radio" name="crowdLimitType" value="0" style="float:left;" <c:if test="${couponCog.crowdLimitType eq '0'}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">不限制</span>
                                        <input type="radio" class="tab-radio" name="crowdLimitType" value="1" style="float:left;" <c:if test="${couponCog.crowdLimitType eq '1'}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">黑名单用户不可领取</span>
                                        
                                        <c:if test="${not empty(groupList)}">
	                                            <span class="blocker en-larger">
	                                            	<input name="crowdLimitType" type="radio" value='2' <c:if test="${couponCog.crowdLimitType eq '2'}"> checked="checked" </c:if>/>
	                                            </span>
	                                            <span class="blocker en-larger">指定群组参与</span>
	                                            <span class="blocker en-larger">
	                                            	<input name="crowdLimitType" type="radio" value='3' <c:if test="${couponCog.crowdLimitType eq '3'}"> checked="checked" </c:if>/>
	                                            </span>
                            ...
                            <c:if test="${not empty(groupList)}"> 
	                                <tr id="userGroupTr" <c:if test="${couponCog.crowdLimitType eq '0' or couponCog.crowdLimitType eq '1'}"> style="display: none;" </c:if>>	
	                                	<td class="ab_left"><label class="title"></label></td>
	                                	<td class="ab_main" colspan="3">
	                                		<div>
		                                        <c:forEach items="${groupList }" var="group">
		                                        	<c:set var="checkedStr" value=""></c:set>
		                                        	<c:forEach items="${fn:split(couponCog.userGroupIds, ',')}" var='groupId'>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <select name="expireRemindDay" class="form-control input-width-larger">
                                            <option value="">请选择</option>
                                            <option value="1" <c:if test="${couponCog.expireRemindDay eq '1'}">selected</c:if>>1</option>
                                            <option value="2" <c:if test="${couponCog.expireRemindDay eq '2'}">selected</c:if>>2</option>
                                            <option value="3" <c:if test="${couponCog.expireRemindDay eq '3'}">selected</c:if>>3</option>
                                            <option value="4" <c:if test="${couponCog.expireRemindDay eq '4'}">selected</c:if>>4</option>
                                            <option value="5" <c:if test="${couponCog.expireRemindDay eq '5'}">selected</c:if>>5</option>
                                            <option value="6" <c:if test="${couponCog.expireRemindDay eq '6'}">selected</c:if>>6</option>
                                            <option value="7" <c:if test="${couponCog.expireRemindDay eq '7'}">selected</c:if>>7</option>
                                        </select>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
