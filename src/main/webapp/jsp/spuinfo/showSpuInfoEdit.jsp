	...
	<script>
		...
		function checkName(bussionName){
			$.ajax({
			    ...
			    data:{
			    		"infoKey":$("input[name='spuInfoKey']").val(),
  ...
  <body>
    ...
    <div class="crumbs">
        <ul id="breadcrumbs" class="breadcrumb">
            ...
            <li class="current"><a title="">编辑SPU</a></li>
        </ul>
    </div>
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
            	action="<%=cpath %>/spuInfo/doSpuInfoEdit.do">
                <input type="hidden" name="queryParam" value="${queryParam}" />
                <input type="hidden" name="pageParam" value="${pageParam}" />
                <input type="hidden" name="spuInfoKey" value="${spuInfo.spuInfoKey}" />
                		...
                		<table class="active_board_table">
                			<tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="spuCode" tag="validate"  value="${spuInfo.spuCode }"
	                       	...
	                       	<tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="spuName" tag="validate"  value="${spuInfo.spuName }"
                			...
                			<tr>
	                       		...
	                       		<td class="ab_main sku" colspan="3">
	                       			<c:if test="${not empty(spuInfo.skuList) }">
	                       				<c:forEach items="${spuInfo.skuList}" var="skuInfo" varStatus="idx">
	                       					<div class="content sku promotionSku" style="display: flex; margin: 5px 0px;">
		                                        <select class="form-control input-width-larger required" name="skuKeyGroup" tag="validate">
		                                            ...
		                                            <option value="${sku.skuKey}" 
		                                            	<c:if test="${sku.skuKey eq skuInfo.skuKey}"> selected="selected" </c:if>>
		                                            	${sku.skuName}
		                                            </c:forEach>
		                                            </c:if>
		                                        </select>
	                       			...
	                       			<c:if test="${empty(spuInfo.skuList) }">
                       					<div class="content sku promotionSku" style="display: flex; margin: 5px 0px;">
	                                        <select class="form-control input-width-larger required" name="skuKeyGroup" tag="validate">
	                                            <option value="">请选择SKU</option>
