  ...
  <body>
    ...
    <div class="crumbs">
        <ul id="breadcrumbs" class="breadcrumb">
            ...
            <li class="current"><a> 修改彩蛋规则</a></li>
        </ul>
    </div>
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
            	action="<%=cpath%>/taoEasterEgg/doTaoEasterEggCogEdit.do">
                <input type="hidden" name="queryParam" value="${queryParam}" />
                <input type="hidden" name="pageParam" value="${pageParam}" />
                <input type="hidden" name="infoKey" value="${easterEggCog.infoKey}" />
                		...
                		<table class="active_board_table">
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="taoName" tag="validate" class="form-control required" 
	                       					value="${easterEggCog.taoName }" style="width: 330px;" autocomplete="off" maxlength="30" />
                        		...
                        		<td class="ab_main date">
                        			<div class="content date" style="margin: 5px 0px; display: flex;">
                                        <input name="startDate" id="startDate" class="Wdate form-control input-width-medium required preTime" 
                                        	value="${easterEggCog.startDate }" tag="validate" autocomplete="off" 
                                        ...
                                        <input name="endDate" id="endDate" class="Wdate form-control input-width-medium required sufTime" 
                                        	value="${easterEggCog.endDate }" tag="validate" autocomplete="off" 
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="taoMemberOrder" tag="validate" class="form-control required" 
                                            value="${easterEggCog.taoMemberOrder }" style="width: 330px;" autocomplete="off" maxlength="200" />
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                               ...
                               <td class="ab_main" colspan="3">
                                   <div class="content">
                                       <select name="ladderType" tag="validate" class="form-control input-width-larger required" >
                                           ...
                                           <option value="1" <c:if test="${easterEggCog.ladderType eq '1' }"> selected = "selected" </c:if>>组合活动</option>
                                       </select>
                                       <label class="validate_tips"></label>
                                   </div>
                               </td>
                           ...
                           <tr>
	                       		...
	                       		<td class="ab_main activity">
	                       			...
	                       			<c:forEach items="${fn:split(easterEggCog.vcodeActivityKeys, ',')}" var="activityKey" varStatus="idx">
	                       	...
	                       	<tr>
                                ...
                                <td class="ab_main ladderItem">
	                       	        <c:forEach items="${fn:split(easterEggCog.ladderRule, ';')}" var="ladderRule" varStatus="idx">
                                    <c:set var="ladderItemAry" value="${fn:split(ladderRule, ':')}"></c:set>
                                    <div class="content ladderItem" style="margin: 5px 0px;">
                                         <span class="blocker en-larger">阶梯</span>
                                         <input name="ladderItem" class="form-control input-width-larger" value="${fn:length(ladderItemAry) > 1 ? ladderItemAry[0] : ''}" placeholder="为空时为默认口令" autocomplete="off" maxlength="100"/>
                                         <span class="blocker en-larger">&nbsp;&nbsp;彩蛋口令</span>
                                         <input name="ladderOrder" class="form-control input-width-larger" value="${fn:length(ladderItemAry) > 1 ? ladderItemAry[1] : ladderItemAry[0]}" placeholder="口令以逗号分隔" style="width: 40% !important;" autocomplete="off" maxlength="1000" />
                                        <label class="title mart5 btn-txt-add-red" style="font-weight: normal; margin-left: 5px;" id="addLadderItem"><c:choose><c:when test="${idx.index eq 0}">新增</c:when><c:otherwise>删除</c:otherwise></c:choose></label>
                                    </div>
	                       	        </c:forEach>
                                </td>
                            </tr>
