	...
	<script>
		$(function(){
			...
			$("#addActivity").click(function(){
				var url = "<%=cpath%>/marquee/showMarqueeCogInfoAdd.do";
			...
			$("a.edit").on("click", function(){
				...
				var url = "<%=cpath%>/marquee/showMarqueeCogInfoEdit.do?infoKey="+key;
				$("form").attr("action", url);
				$("form").submit(); 
			});
			// 删除
    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <div class="widget box">
	            <div class="row">
	               <div class="col-md-2"><span style="font-size: 15px; color: white; font-weight: bold;">跑马灯查询</span></div>
	            ...
	            <div class="widget-content form-inline">
		            <form class="listForm" method="post" action="<%=cpath%>/marquee/showMarqueeList.do">
			            ...
			            <div class="col-md-12 ">
                            <div class="form-group little_distance search" style="line-height: 35px;">
                                <div class="search-item">
			                        ...
			                        <input name="marqueeName" class="form-control input-width-larger" autocomplete="off" maxlength="30"/>
                                </div>
								<div class="search-item">
                        ...
                        <table class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10"
                            ...
                            <thead>
					            <tr>
					            	...
					            	<th style="width:15%;" data-ordercol="sku_name">跑马灯名称</th>
                                    <th style="width:15%;" data-ordercol="sku_name">渠道类型</th>
					            	<th style="width:10%;" data-ordercol="commodity_code">显示时间</th>
					            	<th style="width:10%;" data-ordercol="commodity_code">启用状态</th>
					            	<th style="width:7%;" data-ordercol="valid_day">创建时间</th>
					            	<c:if test="${currentUser.roleKey ne '4'}">
					        ...
					        <tbody>
					        	...
					        	<tr>
					        		...
					        		<td>
					        			<span>${item.marqueeName}</span>
					        		</td>
                                    <td style="text-align:center;">
                                        <c:choose>
                                            <c:when test="${item.winType eq '0'}">扫码抽奖</c:when>
                                            <c:when test="${item.winType eq '2'}">转盘抽奖</c:when>
                                            <c:when test="${item.winType eq '3'}">盲盒抽奖</c:when>
                                            <c:when test="${item.winType eq '4'}">福利站</c:when>
                                            <c:when test="${item.winType eq '5'}">福袋抽奖</c:when>
                                        </c:choose>
                                    </td>
					        		<td>
									...
									<td style="text-align:center;">
										<c:choose>
											<c:when test="${item.status eq '0'}">启用</c:when>
											<c:when test="${item.status eq '1'}">未启用</c:when>
										</c:choose>
									</td>
					        		<td>
