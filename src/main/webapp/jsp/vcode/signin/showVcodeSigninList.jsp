	...
	<script>
		$(function(){
			...
			$("#addActivity").click(function(){
				var url = "<%=cpath%>/vcodeSignin/showVcodeSigninAdd.do";
			...
			$("a.edit").on("click", function(){
				...
				var url = "<%=cpath%>/vcodeSignin/showVcodeSigninEdit.do?activityKey="+key;
				$("form").attr("action", url);
				$("form").submit(); 
			});
			...
			$("a.rule").on("click", function(){
				var key = $(this).parents("td").data("key");
				var url = "<%=cpath%>/vcodeActivityRebateRule/showRebateRuleListMain.do?vcodeActivityKey=" + key + "&activityType=4";
				$("form").attr("action", url);
				$("form").submit(); 
			});
			...
			$("a.view").on("click", function(){
				var key = $(this).parents("td").data("key");
				var url = "<%=cpath%>/vcodeSignin/showVcodeSigninView.do?activityKey="+key;
				$("form").attr("action", url);
				$("form").submit(); 
			});
        });
  ...
  <body>
    ...
    <div class="row">
	    <div class="col-md-12 tabbable">
	        <a href="<%=cpath%>/vcodeSignin/showVcodeSigninList.do?vjfSessionId=${vjfSessionId}&tabsFlag=0" 
	             class="btn <c:if test="${tabsFlag eq '0'}">btn-blue</c:if>">待上线</a>
	        <a href="<%=cpath%>/vcodeSignin/showVcodeSigninList.do?vjfSessionId=${vjfSessionId}&tabsFlag=1" 
	             class="btn <c:if test="${tabsFlag eq '1'}">btn-blue</c:if>">已上线</a>
	        <a href="<%=cpath%>/vcodeSignin/showVcodeSigninList.do?vjfSessionId=${vjfSessionId}&tabsFlag=2" 
	             class="btn <c:if test="${tabsFlag eq '2'}">btn-blue</c:if>">已下线</a>
	        <a href="<%=cpath%>/vcodeSignin/showVcodeSigninList.do?vjfSessionId=${vjfSessionId}&tabsFlag=3" 
    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
        	<div class="widget box">
	            <div class="row">
	               <div class="col-md-2"><span style="font-size: 15px; color: white; font-weight: bold;">万能签到查询</span></div>
	            ...
	            <div class="widget-content form-inline">
		            <form class="listForm" method="post"
	                    action="<%=cpath%>/vcodeSignin/showVcodeSigninList.do">
                        ...
                        <table class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10"
                            ...
                            <thead>
					            <tr>
					            	...
					            	<th style="width:13%;" data-ordercol="a.activity_no">活动编号</th>
					            	<th style="width:27%;" data-ordercol="a.activity_name">活动名称</th>
					            	<th style="width:18%;" data-ordercol="a.start_date">活动时间</th>
					            	<th style="width:13%;" data-ordercol="a.create_time" data-orderdef>创建时间</th>
