...
<div class="container">
    <div class="crumbs">
        <ul id="breadcrumbs" class="breadcrumb">
            ...
            <li class="current"><a title="">修改优惠券</a></li>
        </ul>
    </div>
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
                  action="<%=cpath %>/vcodeTicketExchange/doTicketExchangeEdit.do">
                <input type="hidden" name="infoKey" value="${ticketExchangeCog.infoKey}"/>
                <div class="widget box">
                    <div class="widget-header">
                        <h4><i class="iconfont icon-xinxi"></i>优惠券修改</h4>
                    </div>
                        ...
                        <table class="active_board_table">
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <select name="ticketNo" tag="validate" class="form-control required input-width-larger" disabled>
                                            <option value="">请选择</option>
                                            <c:forEach items="${stringStringMap}" var="item">
                                                <option value="${item.key}" <c:if test="${ticketExchangeCog.ticketNo eq item.key}">selected</c:if>>${item.value}</option>
                                            </c:forEach>
                                        </select>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        ...
                                        <input name="startDate" id="startDate"
                                               ...
                                               tag="validate" autocomplete="off" value="${ticketExchangeCog.startDate}"
                                        ...
                                        <input name="endDate" id="endDate"
                                               ...
                                               tag="validate" autocomplete="off" value="${ticketExchangeCog.endDate}"
                                    ...
                                    <div style="line-height: 35px;" class="content">
                                        <div style="display: flex;">
                                            <div style="float: left; width: 100px; line-height: 30px;">
                                                <input type="radio" name="cashPrize" value="0"
                                                       <c:if test="${ticketExchangeCog.exchangeEndTime != '' and not empty ticketExchangeCog.exchangeEndTime}">checked="checked"</c:if>
                                                       style="float: left; height:20px; cursor: pointer;">
                                                <span>固定截止时间</span>
                                            </div>
                                            <input name="exchangeEndTime" id="exchangeEndTime" value="${ticketExchangeCog.exchangeEndTime}"
                                                   <c:if test="${ticketExchangeCog.exchangeEndTime == '' or empty ticketExchangeCog.exchangeEndTime}">disabled</c:if>
                                                   class="form-control input-width-medium Wdate required preTime"
                                        ...
                                        <div style="display: flex;">
                                            <div style="float: left; width: 110px; line-height: 30px;">
                                                <input type="radio" name="cashPrize" value="1"
                                                       <c:if test="${ticketExchangeCog.exchangeEndDay != '' and not empty ticketExchangeCog.exchangeEndDay}">checked="checked"</c:if>
                                                       style="float: left; height:20px; cursor: pointer;">
                                                <span>兑换后有效天数</span>
                                            </div>
                                            <input name="exchangeEndDay" onkeyup="value=value.replace(/[^\d]/g,'')" id="exchangeEndDay" tag="validate" value="${ticketExchangeCog.exchangeEndDay}"
                                                   <c:if test="${ticketExchangeCog.exchangeEndDay == '' or empty ticketExchangeCog.exchangeEndDay}">disabled</c:if>
                                                   class="form-control required positive number input-width-small rule"
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="vpointsCog" tag="validate" value="${ticketExchangeCog.vpointsCog}"
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div style="float: left; width: 80px; line-height: 25px;">
                                        <input type="radio" name="exchangeLimitRadio"
                                               value="0" class="positive" tag="validate"
                                        <c:if test="${ticketExchangeCog.exchangeLimit == '' or empty ticketExchangeCog.exchangeLimit}">checked="checked"</c:if>
                                        ...
                                        <div style="display: flex;">
                                            <div style="float: left; width: 40px; line-height: 25px;">
                                                <input type="radio" name="exchangeLimitRadio" value="1"
                                                       <c:if test="${ticketExchangeCog.exchangeLimit != '' and not empty ticketExchangeCog.exchangeLimit}">checked="checked"</c:if>
                                                       style="float: left; height:20px; cursor: pointer;">
                                                <span>每人</span>
                                            </div>
                                            <input name="exchangeLimit" id="exchangeLimit" tag="validate"
                                                   class="form-control required number positive input-width-small rule"
                                                   <c:if test="${ticketExchangeCog.exchangeLimit == '' or empty ticketExchangeCog.exchangeLimit}">disabled</c:if>
                                                   <c:if test="${ticketExchangeCog.exchangeLimit != '' and not empty ticketExchangeCog.exchangeLimit}">value="${ticketExchangeCog.exchangeLimit}"</c:if>
