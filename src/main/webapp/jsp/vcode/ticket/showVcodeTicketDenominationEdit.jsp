...
<head>
    ...
    <script>
        ...
        function initPage() {
            ...
            var imgData = '${ticketInfo.ticketPicUrl}';
            if (imgData) {
                showImg(imgData);
            }
    ...
    <div class="row mart20">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
                  action="<%=cpath %>/vpsVcodeTicketDenomination/doTicketDenominationEdit.do">
                    ...
                    <div class="widget-content panel no-padding">
                        ...
                        <table class="active_board_table">
                            <tr style="display:none">
                                <td class="ab_left"><label class="infoKey"><span class="required">*</span></label></td>
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name=infoKey tag="validate"
                                               class="form-control required" autocomplete="off" maxlength="30"
                                               value="${ticketInfo.infoKey}"/>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="ticketNo" tag="validate"
                                               class="form-control required" value="${ticketInfo.ticketNo}" style="display: none">
                                        <input name="categoryType" tag="validate"
                                               class="form-control required" value="${ticketInfo.categoryType}" style="display: none">
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="releaseStartDate" class="form-control input-width-medium required Wdate preTime"
                                               tag="validate" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:'%y-%M-%d'})" value="${ticketInfo.releaseStartDate}" />
                                        <span class="blocker en-larger">至</span>
                                        <input name="releaseEndDate" class="form-control input-width-medium required Wdate sufTime"
                                               tag="validate" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:'%y-%M-%d'})" value="${ticketInfo.releaseEndDate}"/>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" class="tab-radio" name="prizeExpireType" value="0"
                                               style="float:left;"
                                               <c:if test="${ticketInfo.endDate != null and ticketInfo.endDate ne ''}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">指定日期</span>
                                        <input name="endDate"
                                               class="form-control input-width-medium required Wdate sufTime"
                                               <c:if test="${ticketInfo.endDate == null || ticketInfo.endDate eq ''}">disabled="disabled"</c:if>
                                        ...
                                        <input type="radio" class="tab-radio" name="prizeExpireType" value="1"
                                               ...
                                               <c:if test="${ticketInfo.expiredDay != null and ticketInfo.expiredDay ne ''}">checked</c:if>/>
                                        <span class="blocker en-larger" style="margin-left: 2px;">有效天数</span>
                                        <input name="expiredDay" tag="validate"
                                               <c:if test="${ticketInfo.expiredDay == null || ticketInfo.expiredDay eq ''}">disabled="disabled"</c:if>
                                               class="form-control input-width-small required number positive"
                                               autocomplete="off" maxlength="3" value="${ticketInfo.expiredDay}"/>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content firstRebate">
                                        <select class="form-control input-width-larger required" tag="validate" disabled
                                                name="categoryKey">
                                            <c:forEach items="${ticketCategoryList }" var="item">
                                                <option value="${item.categoryKey }"
                                                        <c:if test="${ticketInfo.categoryKey eq item.categoryKey}">
                                                            selected="selected"
                                                        </c:if>>
                                                        ${item.categoryName }
                                                </option>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content firstRebate">
                                        <select name="ticketType" class="form-control input-width-larger search" disabled
                                                autocomplete="off">
                                            <option value="0"  <c:if
                                                    test="${ticketInfo.ticketType eq '0'}"> selected="selected"</c:if>>链接
                                            </option>
                                            <option value="1" <c:if
                                                    test="${ticketInfo.ticketType eq '1'}"> selected="selected"</c:if>>券码
                                            </option>
                                            <%--<option value="2" <c:if
                                                    test="${ticketInfo.ticketType eq '2'}"> selected="selected"</c:if>>图片
                                            </option>--%>
                                            <option value="3" <c:if
                                                    test="${ticketInfo.ticketType eq '3'}"> selected="selected"</c:if>>动态券码
                                            </option>
                                            <option value="4" <c:if
                                                    test="${ticketInfo.ticketType eq '4'}"> selected="selected"</c:if>>活动编码
                                            </option>
                                        </select>
                                        <label class="validate_tips"></label>
                                    </div>
                            ...
                            <tr id="urlTr" <c:if test="${ticketInfo.ticketType eq '1'}">style="display: none"</c:if>>
                                <td class="ab_left"><label class="title">优惠券链接：<span class="required">*</span></label>
                                </td>
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input id="ticketUrl" name="ticketUrl" tag="validate"
                                               class="form-control required" autocomplete="off" maxlength="1000" PLACEHOLDER="请输入URL" value="${ticketInfo.ticketUrl}"/>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            <tr id="fileTr" <c:if test="${ticketInfo.ticketType ne '1'}">style="display: none"</c:if>>
                                <td class="text-right"><label class="title">优惠券信息：<span class="required">*</span></label></td>
                                <td>
                                    <div class="content">
                                        <input type="file"  class="import-file" id="ticketFile" name="ticketFile" single style="padding-bottom: 15px; margin-left: 10px;" disabled/>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td class="ab_left"><label class="title">优惠券面额：<span class="required">*</span></label></td>
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="ticketMoney" tag="validate"
                                               class="form-control maxValue money required" autocomplete="off" maxlength="100" value="${ticketInfo.ticketMoney}" />
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                                    ...
                                    <div class="content">
                                        <div class="content">
                                            <textarea tag="validate" rows="5" class="form-control required"
                                                      autocomplete="off" name="ticketDesc"
                                                      maxlength="500">${ticketInfo.ticketDesc}</textarea>
                                            <label class="validate_tips"></label>
                                        </div>
                                    </div>
                                </td>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="ticketStatus" type="hidden" value="${ticketInfo.ticketStatus}" />
                                        <input id="ticketStatus" type="checkbox" <c:if test="${ticketInfo.ticketStatus eq '1'}">checked</c:if> data-size="small" data-on-text="启用" data-off-text="停用" data-on-color="success" data-off-color="warning"/>
                                    </div>
                                </td>
                            </tr>
