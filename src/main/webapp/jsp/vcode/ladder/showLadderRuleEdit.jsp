  ...
  <head>
	...
	<script>
		...
		$(function(){
            ...
            $("[name='ruleFlag']").trigger("change");
            
            // 回显阶梯时间
            var dbLadderStartTime = '${ladder.ladderStartTime }';
            var dbLadderEndTime = '${ladder.ladderEndTime }';
            if(dbLadderStartTime == ''){
            	$("input[name='ladderStartTime']").val("00:00:00");
            }else{
            	$("input[name='ladderStartTime']").val(dbLadderStartTime);
            }
            
            if(dbLadderEndTime == ''){
            	$("input[name='ladderEndTime']").val("23:59:59");
            }else{
            	$("input[name='ladderEndTime']").val(dbLadderEndTime);
            }
  ...
  <body>
    ...
    <div class="crumbs">
        <ul id="breadcrumbs" class="breadcrumb">
            ...
            <li class="current"><a>修改阶梯规则</a></li>
        </ul>
    </div>
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
            	action="<%=cpath%>/ladderRule/doLadderRuleEdit.do">
                <input type="hidden" name="queryParam" value="${queryParam}" />
                <input type="hidden" name="pageParam" value="${pageParam}" />
                <input type="hidden" name="infoKey" value="${ladder.infoKey}" />
                	...
                	<div class="widget-content panel no-padding">
                		<table class="active_board_table">
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="ruleName" tag="validate" class="form-control required" 
	                       					value="${ladder.ruleName }" style="width: 330px;" autocomplete="off" maxlength="30" />
                        		...
                        		<td class="ab_main date">
                        			<div class="content date" style="margin: 5px 0px; display: flex;">
                                        <input name="startDate" id="startDate" class="Wdate form-control input-width-medium required preTime" 
                                        	value="${ladder.startDate }" tag="validate" autocomplete="off" 
                                        	onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', maxDate:'#F{$dp.$D(\'endDate\')}'})" disabled="disabled"/>
                                        <span class="blocker en-larger">至</span>
                                        
                                        <input name="endDate" id="endDate" class="Wdate form-control input-width-medium required sufTime" 
                                        	value="${ladder.endDate }" tag="validate" autocomplete="off"  <c:if test="${ladder.isBegin=='2' }">disabled="disabled"</c:if>
                			...
                			<tr>
                               ...
                               <td class="ab_main" colspan="3">
                                   <div class="content">
                                       <select name="ruleFlag" tag="validate" class="form-control input-width-larger required" disabled="disabled">
                                           <option value="1">每日</option>
                                           <option value="2" <c:if test="${ladder.ruleFlag eq '2' }"> selected = "selected" </c:if>>时间段
                                       </select>
                                       <label class="validate_tips"></label>
                                   </div>
                               </td>
                           ...
                           <tr class="ladderTimeTr">
                               ...
                               <td class="ab_main time">
                    			<div class="content">
                                    ...
                                    onfocus="WdatePicker({dateFmt:'HH:mm:ss', maxDate:'#F{$dp.$D(\'ladderEndTime\')}'})" value="00:00:00" 
                                    autocomplete="off" disabled="disabled"/>
                                    <span class="blocker en-larger">至</span>
                                    <input name="ladderEndTime" id="ladderEndTime" class="form-control input-width-medium Wdate sufTime" 
                                    onfocus="WdatePicker({dateFmt:'HH:mm:ss', minDate:'#F{$dp.$D(\'ladderStartTime\')}'})" value="23:59:59" 
                                    autocomplete="off" disabled="disabled"/>
                                    <label class="validate_tips"></label>
                    			</div>
                    		</td>
                            ...
                            <tr>
	                       		...
	                       		<td class="ab_main activity">
	                       			...
	                       			<c:forEach items="${fn:split(ladder.vcodeActivityKeys, ',')}" var="activityKey" varStatus="idx">
		                       			<div class="content activity" style="margin: 5px 0px; display: flex;">
		                       				...
		                       				<c:choose>
		                       					<c:when test="${idx.index > 0 }">
		                       						<label class="title mart5 btn-txt-add-red" style="font-weight: normal; margin-left: 5px;" id="addActivity">删除</label>
		                       					</c:when>
		                       					<c:otherwise>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div style="float: left; width: 80px; line-height: 25px;">
                                        <input type="radio" name="allowanceaExclusiveFlag" value="0" style="float: left; height:20px; cursor: pointer;" <c:if test= "${ladder.allowanceaExclusiveFlag eq '0'}"> checked="checked" </c:if>>&nbsp;不互斥
                                    </div>
                                    <div style="float: left; width: 80px; line-height: 25px;">
                                        <input type="radio" name="allowanceaExclusiveFlag" value="1" style="float: left; height:20px; cursor: pointer;" <c:if test= "${ladder.allowanceaExclusiveFlag eq '1'}"> checked="checked" </c:if>>&nbsp;互斥
                                    </div>
                                    <label class="validate_tips"></label>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <textarea class="form-control" maxlength="4000" tag="validate" autocomplete="off" name="remarks">${ladder.remarks}</textarea>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
