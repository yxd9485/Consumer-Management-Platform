    ...
    <script>
        $(function(){
            ...
            $("#addItem").click(function(){
                var url = "<%=cpath%>/ladderUI/showLadderUIAdd.do.do?ruleKey="+'${ruleKey}'
            ...
            $("a.edit").on("click", function(){
                ...
                var url = "<%=cpath%>/ladderUI/showLadderUIEdit.do?info="+key;
                $("form").attr("action", url);
                $("form").submit();
            });
            ...
            $("a.del").on("click", function(){
                var key = $(this).parents("td").data("key");
                var url = "<%=cpath%>/ladderUI/doLadderUIDel.do?info="+key+"&ruleKey="+'${ruleKey}';
    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <div class="widget box" id="tab_1_1">
                <div class="row">
                    <div class="col-md-2"><span style="font-size: 15px; color: white; font-weight: bold;">UI列表</span></div>
                ...
                <div class="widget-content form-inline">
                    <form class="listForm" method="post"
                          action="<%=cpath%>/ladderUI/showLadderUIList.do">
                    ...
                    <div role="grid" class="dataTables_wrapper form-inline" id="DataTables_Table_0_wrapper">
                        <table class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10"
                            ...
                            <tr>
                                ...
                                <th style="width:8%;" data-ordercol="rule_no">阶梯类型</th>
                                <th style="width:8%;" data-ordercol="rule_no">阶梯级数</th>
                                <th style="width:18%;" data-ordercol="rule_name">中奖图片</th>
                                <th style="width:18%;" data-ordercol="rule_name">阶梯描述</th>
                                ...
                                <c:when test="${fn:length(ladderUIList) gt 0}">
                                    <c:forEach items="${ladderUIList}" var="item" varStatus="idx">
                                        <tr>
                                            ...
                                            <td style="text-align:center;">
                                                <c:choose>
                                                    <c:when test="${item.ladderType eq 0}">按倍数中出</c:when>
                                                    <c:otherwise>按支数中出</c:otherwise>
                                                </c:choose>
                                            </td>
                                            <td style="text-align:center;">
                                                <c:choose>
                                                    <c:when test="${item.ladderType eq 0}">${item.ladderMultiple}</c:when>
                                                    <c:otherwise>${item.ladderStNum} 至 ${item.ladderEndNum}</c:otherwise>
                                                </c:choose>
                                            </td>
                                            <td>
                                                <span><img src="https://img.vjifen.com/images/vma/${item.ladderPic}" style="height: 80px;width: 170px; float: left;"></span>
                                            </td>

                                            <td style="text-align:center;">
                                                <span>${item.ladderDesc}</span>
                                            </td>
                                            <td style="text-align:center;">
