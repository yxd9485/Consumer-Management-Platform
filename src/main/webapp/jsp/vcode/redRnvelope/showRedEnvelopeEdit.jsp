...
// 			}
        ...
        function initPage() {
            ...
            $("form").on("change", "div.area select", function(){
                if ($(this).val() != "") {
                    ...
                    $("td.area div.area").each(function(i){
                        ...
                        var $county = $(this).find("select.zDistrict");
                        if ($county.val() != "") {
                            areaCode2 = areaCode2 + $county.val() + ",";
                        } else if ($city.val() != "") {
                            areaCode2 = areaCode2 + $city.val() + ",";
                        } else if ($province.val() != "") {
                            areaCode2 = areaCode2 + $province.val() + ",";
                        }
                    });

                    // 加载区域对应的热区
                    findHotAreaListByAreaCode(areaCode2);

                }
            });
            var imgData = '${entity.ruleContentUrl}';
            ...
            <%--let isBegin = ${isBegin}--%>
            // 初始化筛选区域
            var filterAreaCodeAry = "${entity.areaCode}".split(",");
            console.log("filterAreaCodeAry",filterAreaCodeAry)
        ...
        function formatNum(num,that) {
            ...
            if (arrayNum.length == 1) {
                $(that).val(value.toStrng() + ".00")
                return value.toStrng() + ".00";
            }
            if (arrayNum.length > 1) {
                //小数点右侧 如果小于两位 则补一个0
                if (arrayNum[1].length < 2) {
                    $(that).val(value.toString() + "0")
    ...
    <div class="row mart20">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
                  action="<%=cpath %>/redEnvelopeRain/redEnvelopeUpdate.do">
                        ...
                        <table class="active_board_table">
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="statusFlag" type="hidden"  value="${entity.statusFlag}" />
                                        <input id="statusFlag" type="checkbox" <c:if test="${entity.statusFlag eq '1'}">checked</c:if> data-size="small" data-on-text="开"  data-off-text="关" data-on-color="info" data-off-color="danger"/>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="activityName" tag="validate"
                                               class="form-control input-width-larger  required" autocomplete="off" maxlength="25" value="${entity.activityName}" />
                            ...
                            <tr>
                                ...
                                <td class="ab_main area" colspan="3">
                                    <div class="content area" style="display: flex; margin: 5px 0px;">
                                        ...
                                        <label class=" title mart5 btn-txt-add-red addArea" id="addArea"
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="startDate" id="startDate" disabled value="${entity.startDate}"
                                        ...
                                        <input name="endDate" id="endDate"
                                               ...
                                               tag="validate" autocomplete="off" value="${entity.endDate}"
                            ...
                            <tr>
                                ...
                                <td class="ab_main time">
                                    <c:forEach items="${entity.beginTime}" var="item"  varStatus="idx" >
                                        <div class="content time" style="margin: 5px 0px;">
                                            ...
                                            <input name="beginTime" id="beginTime0" class="form-control input-width-medium Wdate"
                                                   onfocus="WdatePicker({dateFmt:'HH:mm:ss', maxDate:'#F{$dp.$D(\'endTime0\')}'})" value="${entity.beginTime[idx.index]}" autocomplete="off"/>
                                            <span class="blocker en-larger">至</span>
                                            <input name="endTime" id="endTime0" class="form-control input-width-medium Wdate addTime"
                                                   onfocus="WdatePicker({dateFmt:'HH:mm:ss', minDate:'#F{$dp.$D(\'beginTime0\')}'})" value="${entity.endTime[idx.index]}" autocomplete="off"/>
                                            <label class="title mart5 btn-txt-add-red" style="font-weight: normal; margin-left: 5px;" id="addTime"><c:choose><c:when test="${idx.index eq 0}">新增</c:when><c:otherwise><span style="color: red">删除</span></c:otherwise></c:choose></label>
                                        </div>
                                    </c:forEach>
                                        ...
                                        <select id="hotAreaKey" name="hotAreaKey" class="hotArea form-control" style="width: 352px;">
                                            ...
                                            <c:if test="${not empty(hotAreaList) }">
                                                <c:forEach items="${hotAreaList}" var="hotArea">
                                                    <option value="${hotArea.hotAreaKey }" <c:if test="${hotArea.hotAreaKey eq entity.hotAreaKey }">selected</c:if>>${hotArea.hotAreaName}</option>
                                                </c:forEach>
                                            </c:if>
                                        </select>
                            ...
                            <tr id="shareVcodeActivityTr">
                                ...
                                <td class="ab_main sku" colspan="3">
                                    <c:forEach items="${entity.vcodeActivityKeyArray}" var="item"  varStatus="idx">
                                            ...
                                            <select class="form-control input-width-larger required addVcodeActitivy" id="vcodeActivityKey" name="vcodeActivityKey" tag="validate">
                                                ...
                                                <c:if test="${!empty shareActivityKeyList}">
                                                    <c:forEach items="${shareActivityKeyList}" var="activity">
                                                        <option value="${activity.vcodeActivityKey}" <c:if test="${activity.vcodeActivityKey eq item }">selected</c:if>>${activity.vcodeActivityName}</option>
                                                    </c:forEach>
                                                </c:if>
                                            </select>
                                            <span class="blocker en-larger"></span>
                                            <label class="title mart5 btn-txt-add-red" style="font-weight: normal; margin-left: 5px;" id="addVcodeActitivy"><c:choose><c:when test="${idx.index eq 0}">新增</c:when><c:otherwise><span style="color: red">删除</span></c:otherwise></c:choose></label>
                                            <label class="validate_tips"></label>
                                        </div>
                                    </c:forEach>
                                ...
                                <td class="ab_main" colspan="5">
                                    <table id="commonScanPrize" class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10" style="width: 97%; margin: 0 auto; text-align: center; overflow: auto !important">
                                        ...
                                        <tr>
                                            ...
                                            <th style="width:9%; text-align: center;" class="cogItemVpoints">中出</th>
                                            <th style="width:9%; text-align: center;" class="cogItemVpoints">操作</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        <c:choose>
                                            <c:when test="${rebateRuleCogList != null}">
                                                <c:forEach items="${rebateRuleCogList.vpointsCogLst}" var="item" varStatus="idx">
                                                    <tr>
                                                        <td>
                                                            ...
                                                            <select id="randomType" name="randomType" class="form-control input-width-small" style="display: initial; width: 60px !important;">
                                                                <option value="0" <c:if test="${item.randomType eq '0'}">selected</c:if>>随机</option>
                                                                <option value="1"  <c:if test="${item.randomType eq '1'}">selected</c:if>>固定</option>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <div class="random-prize content" <c:if test="${item.randomType eq '1'}">style="display: none;"</c:if> >
                                                                <input type="text" name="minMoney" class="form-control input-width-small number maxValue"  autocomplete="off"
                                                                       tag="validate" data-oldval="0.00" value="<fmt:formatNumber type="number" value="${item.minMoney}" pattern="0.00" maxFractionDigits="2"/>" maxVal="9999.99" maxlength="7" style="display: initial; width: 60px !important;">
                                                                ...
                                                                <input type="text" name="maxMoney" class="form-control input-width-small number maxValue"  autocomplete="off"
                                                                       tag="validate" data-oldval="0.00" value="<fmt:formatNumber type="number" value="${item.maxMoney}" pattern="0.00" maxFractionDigits="2"/>"  maxVal="9999.99" maxlength="7" style="display: initial; width: 60px !important;">
                                                            </div>
                                                            <div class="fixation-prize"  <c:if test="${item.randomType eq '0'}">style="display: none;"</c:if>>
                                                                <input type="text" name="fixationMoney" class="form-control input-width-small number maxValue"  autocomplete="off"
                                                                       tag="validate" data-oldval="0.00" value="<fmt:formatNumber type="number" value="${item.minMoney}" pattern="0.00" maxFractionDigits="2"/>"  maxVal="9999.99" maxlength="7" style="display: initial; width: 132px !important;">
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="unitMoney" id="unitMoney" readonly="readonly"  class="form-control input-width-small number integer maxValue unitMoney"  autocomplete="off"
                                                                   tag="validate" data-oldval="0" value="${(item.maxMoney+item.minMoney) /2}" maxVal="999999" maxlength="6" style="display: initial; width: 132px !important;">
                                                        </td>
                                                        <td style="position: relative;">
                                                            <div style="display: inline-flex;">
                                                                <input type="text" id="prizePercent" name="prizePercent" class="form-control input-width-small number maxValue"
                                                                       autocomplete="off" tag="validate" data-oldval="0.0000" value="${item.prizePercent}" maxVal="100" maxlength="7" style="display: initial; width: 60px !important;">
                                                                <label style="line-height: 30px;">%</label>
                                                            </div>
                                                        </td>
                                                        <td style="position: relative;">
                                                            <div style="display: inline-flex;">
                                                                <input type="text" readonly="readonly"  class="form-control input-width-small number integer "  autocomplete="off"
                                                                       tag="validate" data-oldval="0" value="${item.cogAmounts-item.restAmounts}" maxVal="9999999999" style="display: initial; width: 132px !important;">
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <label id="addPrizeItem" class="btn-txt-add-red"><c:choose><c:when test="${idx.index eq 0}">新增</c:when><c:otherwise><span style="color: red">删除</span></c:otherwise></c:choose></label>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
                                            </c:when>
                                        ...
                                        <table  class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10" style="width: 97%;    margin: 0 auto; text-align: center; overflow: auto !important">
                                            ...
                                            <tr>
                                                ...
                                                <td style="width:9%; text-align: center;" ><span id="totalPrizePercent" data-currval="0" style="color: green;">100.0000%</span></td>
                                                <td style="width:9%; text-align: center;"  class="">--</td>
                    ...
                    <div class="widget-content panel no-padding filteruser">
                        <table class="active_board_table">
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" class="tab-radio" name="restrictTimeType" value="0" style="float:left;"  <c:if test="${'0' eq entity.restrictTimeType }"> checked="checked"</c:if>  />
                                        <span class="blocker en-larger" style="margin-left: 2px;">活动时间</span>
                                        <input type="radio" class="tab-radio" name="restrictTimeType" value="1" style="float:left;" <c:if test="${'1' eq entity.restrictTimeType }"> checked="checked"</c:if> />
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="restrictMoney"  type="text" value="${entity.restrictMoney}"
                                        ...
                                        <span class="blocker en-larger">元（已消费金额：${entity.useRestrictMoney}元，锁定资金数据每2小时更新一次）</span>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="restrictBottle"  type="text" value="${entity.restrictBottle}"
                                               class="form-control input-width-larger integer required" autocomplete="off" maxlength="9" tag="validate"/>
                                        <span class="blocker en-larger">瓶（已消费瓶数：${rebateRuleCogList.consumeBottle}瓶，数据每5分钟更新一次）</span>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="restrictUserCount"  type="text" value="${entity.restrictUserCount}"
