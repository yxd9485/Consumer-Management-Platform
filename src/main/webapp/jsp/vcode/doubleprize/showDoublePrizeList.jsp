  ...
  <body>
    ...
    <div class="row">
	    <div class="col-md-12 tabbable">
	        <a href="<%=cpath%>/doubleprize/showDoublePrizeList.do?vjfSessionId=${vjfSessionId}&tabsFlag=0" 
	             class="btn <c:if test="${tabsFlag eq '0'}">btn-blue</c:if>">待上线</a>
	        <a href="<%=cpath%>/doubleprize/showDoublePrizeList.do?vjfSessionId=${vjfSessionId}&tabsFlag=1" 
	             class="btn <c:if test="${tabsFlag eq '1'}">btn-blue</c:if>">已上线</a>
	        <a href="<%=cpath%>/doubleprize/showDoublePrizeList.do?vjfSessionId=${vjfSessionId}&tabsFlag=2" 
	             class="btn <c:if test="${tabsFlag eq '2'}">btn-blue</c:if>">已下线</a>
	        <a href="<%=cpath%>/doubleprize/showDoublePrizeList.do?vjfSessionId=${vjfSessionId}&tabsFlag=3" 
        	...
        	<div class="widget box">
	            ...
	            <div class="widget-content form-inline">
		            <form class="listForm" method="post"
	                    action="<%=cpath%>/doubleprize/showDoublePrizeList.do">
                        ...
                        <table class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10"
                            ...
                            <thead>
					            <tr>
					            	...
					            	<th style="width:9%;" data-ordercol="a.activity_no">活动编号</th>
					            	<th style="width:15%;" data-ordercol="a.activity_name">活动名称</th>
					            	<th style="width:12%;" data-ordercol="a.start_date">活动时间</th>
					            	<th style="width:7%;" data-ordercol="a.period_type">周期类型</th>
					            	<th style="width:7%;" data-ordercol="a.filter_type">筛选类型</th>
					            	<th style="width:9%;" data-ordercol="a.filter_person_num">符合/参与人数</th>
					            	<th style="width:8%;">抽奖/放弃次数</th>
					        ...
					        <tbody>
					        	...
					        	<tr>
                                    ...
                                    <td style="text-align:center;">
                                        <c:choose>
                                            <c:when test="${activity.periodType eq '0'}">自然周</c:when>
                                            <c:when test="${activity.periodType eq '1'}">自然月</c:when>
                                            <c:when test="${activity.periodType eq '2'}">自然天</c:when>
                                            <c:when test="${activity.periodType eq '3'}">活动周期</c:when>
                                        </c:choose>
                                    </td>
                                    <td style="text-align:center;">
                                        <c:choose>
                                            <c:when test="${activity.filterType eq '0'}">过往用户</c:when>
                                            <c:when test="${activity.filterType eq '1'}">所有用户</c:when>
                                        </c:choose>
                                    </td>
					        		<td style="text-align: right;">
					        			<span title="">${activity.filterPersonNum}/${activity.joinPersonNum}</span>
					        		</td>
					        		<td style="text-align: right;">
					        			<span>${activity.lotteryNum}/${activity.cancelLotteryNum}</span>
					        		</td>
					        		<td style="text-align:center;">
					        			<c:choose>
