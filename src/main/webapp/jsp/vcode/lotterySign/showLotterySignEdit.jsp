    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
                  action="<%=cpath %>/lotterySign/doLotterySignEdit.do">
                <input type="hidden" name="queryParam" value="${queryParam}"/>
                <input type="hidden" name="pageParam" value="${pageParam}"/>
                <input type="hidden" name="infoKey" id="infoKey" value="${vpsLotterySignCog.infoKey}">
                <input type="hidden" name="status" value="${vpsLotterySignCog.status}">
                <input type="hidden" name="noEditStartDate" value="${vpsLotterySignCog.startDate}">
                ...
                <div class="widget box">
                        ...
                        <table class="active_board_table">
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="activityName" tag="validate" value="${vpsLotterySignCog.activityName}"
                                               class="form-control input-width-larger required varlength"
                                               <c:if test="${vpsLotterySignCog.status ne '0'}">disabled</c:if>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        ...
                                        <input name="startDate" id="startDate" value="${vpsLotterySignCog.startDate}"
                                               class="form-control input-width-medium Wdate required preTime"
                                               tag="validate" autocomplete="off" <c:if test="${vpsLotterySignCog.status ne '0'}">disabled</c:if>
                                               onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"/>
                                        <span class="blocker en-larger">至</span>
                                        <input name="endDate" id="endDate"  value="${vpsLotterySignCog.endDate}"
                    ...
                    <div class="widget-content panel no-padding">
                        ...
                        <table class="active_board_table">
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <select name="cycleType" class="form-control input-width-larger search" <c:if test="${vpsLotterySignCog.status ne '0'}">readonly="true" </c:if>
                                                autocomplete="off">
                                            <option value="1" <c:if test="${vpsLotterySignCog.cycleType eq '1'}">selected</c:if> >月</option>
                                            <%--<option value="0" <c:if test="${vpsLotterySignCog.cycleType eq '0'}">selected</c:if> >周</option>--%>
                                        </select>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        ...
                                        <input name="scanCountCog" tag="validate" value="${vpsLotterySignCog.scanCountCog}"
                                               class="form-control input-width-larger required number positive minValue maxValue"
                                               style="width: 500px;" autocomplete="off" maxlength="30" minVal="1" <c:if test="${vpsLotterySignCog.status ne '0'}">disabled</c:if>
                                               maxVal="1000"/>
                            ...
                            <tr>
                                ...
                                <td class="ab_main skuTdCog" colspan="3">
                                    <c:if test="${not empty(vpsLotterySignCog.skuCog) }">
                                        <c:forEach items="${fn:split(vpsLotterySignCog.skuCog,',')}" var="item" varStatus="idx">
                                            <div class="content sku promotionSku" style="display: flex; margin: 5px 0px;">
                                                <select class="form-control input-width-larger required" name="skuCog" tag="validate">
                                                    <option value="">请选择</option>
                                                    <option <c:if test="${item eq '000000'}">selected</c:if> value="000000">全部</option>
                                                    <c:forEach items="${skuList}" var="sku">
                                                    <option value="${sku.skuKey}"
                                                    <c:if test="${item eq sku.skuKey}"> selected="selected" </c:if>>
                                                            ${sku.skuName}
                                                        </c:forEach>
                                                </select>
                                                <c:if test="${idx.index eq 0}">
                                                    <label class="title mart5 addPrize btn-txt-add-red" <c:if test="${item eq '000000'}">style="display: none"</c:if> style="font-weight: normal; margin-left: 5px;" id="addPrize">新增</label>
                                                </c:if>
                                                <c:if test="${idx.index > 0}">
                                                    <label class="title mart5 addPrize btn-txt-add-red" <c:if test="${item eq '000000'}">style="display: none"</c:if> style="font-weight: normal; margin-left: 5px;" id="addPrize">删除</label>
                                                </c:if>
                                                <label class="validate_tips"></label>
                                            </div>
                                        </c:forEach>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="summernote">
                                        ${vpsLotterySignCog.activityRule}
                                    </div>
                                </td>
                            </tr>
                        </table>
                    ...
                    <div class="active_table_submit mart20">
                        <div class="button_place">
                            <c:if test="${vpsLotterySignCog.status ne '2'}">
