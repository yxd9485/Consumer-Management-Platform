    ...
    <script>
        $(function(){
            ...
            $("a.codeOrder").on("click", function(){
                var url = "<%=cpath%>/invitationOrder/showOrderLst.do?vjfSessionId=${vjfSessionId}&tabsFlag=0&activityKey=${activityKey}";
            ...
            $(".export").on("click", function(){
                $.fn.confirm("确定要导出吗？", function(){
                    $("form").attr("action", "<%=cpath%>/invitationCode/uploadCode.do?uploadCodeType=0");
                    $("form").submit();
                    // 还原查询action
                    $("form").attr("action", "<%=cpath%>/invitationCode/showCodeLst.do");
                });
            });
        });
...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-12 tabbable">
            ...
            <a href="<%=cpath%>/invitationCode/showCodeLst.do?vjfSessionId=${vjfSessionId}&tabsFlag=1&activityKey=${activityKey}"
               class="btn <c:if test="${tabsFlag eq '1'}">btn-blue</c:if>">店主码</a>
            <a href="<%=cpath%>/invitationCode/showCodeLst.do?vjfSessionId=${vjfSessionId}&tabsFlag=2&activityKey=${activityKey}"
               class="btn <c:if test="${tabsFlag eq '2'}">btn-blue</c:if>">导购码</a>
        </div>
    </div>
        ...
        <div class="col-md-12 tabbable tabbable-custom">
            <div class="widget box">
                ...
                <div class="widget-content form-inline">
                    <form class="listForm" method="post" action="<%=cpath%>/invitationCode/showCodeLst.do">
                                ...
                                <div class="form-group little_distance search"  style="line-height: 35px;">
                                    ...
                                    <div class="search-item">
                                        <c:if test="${tabsFlag  eq '1'}">
                                            ...
                                            <input name="registrantShopName" class="form-control input-width-larger" autocomplete="off" maxlength="30"/>
                                        </c:if>
                                        <c:if test="${tabsFlag eq '2'}">
                                            <label class="control-label">姓名：</label>
                                            <input name="registrantName" class="form-control input-width-larger" autocomplete="off" maxlength="30"/>
                                        </c:if>
                                    </div>

                                    <div class="search-item">
                                        <label class="control-label">手机号：</label>
                                        <input name="registrantPhoneNum" class="form-control input-width-larger" autocomplete="off" maxlength="30"/>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <c:when test="${fn:length(resultList) gt 0}">
                                        <c:forEach items="${resultList}" var="item" varStatus="idx">
                                            <tr>
                                                ...
                                                <td style="text-align:center;">
                                                    <span>
                                                        <c:if test="${tabsFlag eq '1'}">${item.registrantShopName}</c:if>
                                                        <c:if test="${tabsFlag eq '2'}">${item.registrantName}</c:if>
                                                    </span>
                                                </td>
                                                <td style="text-align:center;">
                                                    <span>${item.registrantPhoneNum}</span>
                                                </td>
                                                <td style="text-align:center;">
