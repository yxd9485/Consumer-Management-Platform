    ...
    <script>
        ...
        function initPage() {
            ...
            $("input[name='activityName']").on("blur",function(){
                ...
                checkName(infoKey,activityName);
            });
            ...
            $("form").on("click", "#addPrizeItem", function(){
                if ($(this).text() == '新增') {
                    ...
                    $cloneItem.find("select[name='rewardType']").val("0"); // 将激励类型设置为“现金”
            ...
            // 监听奖励类型选择框的 change 事件
            $('select[name=rewardType]').on('change', function() {
                // 获取当前选中的选项值
                var selectedValue = $(this).val();
                console.log("=======================监听奖励类型选择框的 change 事件selectedValue:"+selectedValue)

                // 判断选项值，根据值切换激励范围的显示和隐藏
                if (selectedValue === '0') {  // 现金
                    $(this).closest('tr').find('#moneyCog').show();
                    var vpointsCog = $(this).closest('tr').find('#vpointsCog');
                    vpointsCog.hide();
                    vpointsCog.find('[name=minVpoints]').val('0');
                    vpointsCog.find('[name=maxVpoints]').val('0');
                } else if (selectedValue === '1') {  // 积分
                    $(this).closest('tr').find('#vpointsCog').show();
                    var moneyCog = $(this).closest('tr').find('#moneyCog');
                    moneyCog.hide();
                    moneyCog.find('[name=minMoney]').val('0.00');
                    moneyCog.find('[name=maxMoney]').val('0.00');
                }
            });
        ...
        function checkName(infoKey,bussionName){
            $.ajax({
                url : "${basePath}/invitationWithGifts/checkName.do",
                data:{
                    "infoKey":infoKey,
        ...
        function validForm() {
            ...
            var checkResult = checkDateAndArea();
            if(checkResult!='0'){
                $.fn.alert("当前地区已有未结束的活动：" + checkResult);
                return false;
            }
            ...
            $("#newUserPrize tbody tr").each(function(index) {
                ...
                var rewardType = $(this).find("select[name='rewardType']").val();
                var minMoney = $(this).find("input[name='minMoney']").val();
                var maxMoney = $(this).find("input[name='maxMoney']").val();
                var minVpoints = $(this).find("input[name='minVpoints']").val();
                var maxVpoints = $(this).find("input[name='maxVpoints']").val();

                // 根据 rewardType 的值进行数据检查处理
                if (rewardType == 0) {
                    ...
                    if (ladder == "" || ladder == null
                        || numOfPeople == "" || numOfPeople == null
                        || minVpoints == "" || minVpoints == null
                        || maxVpoints == "" || maxVpoints == null) {
                        check = false;
                        checkMsg = "新用户奖励设置第" + (index + 1) + "行中有信息需要完善！";
                        return false; // 停止遍历
                    }else if(maxVpoints <= 0){
                        check = false;
                        checkMsg = "新用户奖励设置第" + (index + 1) + "行中最大积分需大于0！";
                        return false; // 停止遍历
                    }
                    minMoney = "0.00";
                    maxMoney = "0.00";
                ...
                newUserRowData += ladder + "," + numOfPeople + "," + rewardType + "," + minMoney + "," + maxMoney + "," + minVpoints + "," + maxVpoints + ";";
            });
            if (!check){
                $.fn.alert(checkMsg);
                return false;
                ...
                $("#oldUserPrize tbody tr").each(function(index) {
                    ...
                    if (rewardType == 0) {
                        ...
                        if (ladder == "" || ladder == null
                            ...
                            checkOld = false;
                            checkMsgOld = "老用户奖励设置第" + (index + 1) + "行中最大积分需大于0！";
                            return false; // 停止遍历
                        }
                        minMoney = "0.00";
                        maxMoney = "0.00";
                    ...
                    // 将这些值拼接成一个逗号分隔的字符串，并添加到 rowData 中
                    oldUserRowData += ladder + "," + numOfPeople + "," + rewardType + "," + minMoney + "," + maxMoney + "," + minVpoints + "," + maxVpoints + ";";
                });
                if (!checkOld){
        ...
        function checkDateAndArea(){
            var infoKey = $("input[name='infoKey']").val();
            var startTime = $("input[name=startTime]").val()
            var endTime = $("input[name=endTime]").val()
            var area = $("#area").val();
            var checkResult = '区域检查错误';//0是没有
            $.ajax({
                url : "${basePath}/invitationWithGifts/checkDateAndArea.do",
                data:{
                    "startTime":startTime,
                    "endTime":endTime,
                    "area":area,
    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
                  action="<%=cpath %>/invitationWithGifts/editActivity.do">
                <input type="hidden" name="infoKey" id="infoKey" value="${entity.infoKey}" />
                ...
                <div class="widget box">
                        ...
                        <table class="active_board_table">
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="status" type="hidden" value="${entity.status}" />
                                        <input id="status" type="checkbox" <c:if test="${entity.status eq '1'}">checked</c:if> data-size="small" data-on-text="启用" data-off-text="停用" data-on-color="success" data-off-color="warning"/>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="activityName" tag="validate" value="${entity.activityName}"
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="shortName" tag="validate" value="${entity.shortName}"
                                               class="form-control input-width-larger required varlength"
                                               autocomplete="off" data-length="10" maxlength="8"/>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="startTime" id="startTime" value="${entity.startTime}"
                                        ...
                                        <input name="endTime" id="endTime" value="${entity.endTime}"
                                ...
                                <td class="ab_main" colspan="3">
                                    <div style="height: 163px;width: 800px; float: left;" class="img-box full">
                                        <section class="img-section" data-imgnum="4">
                                            <input id="imgUrl" name="prizeImgAry" type="hidden" class='filevalue' value="${entity.backgroundUrl}"/>
                            ...
                            <tr style="height: 163px;">
                                ...
                                <td class="ab_main" colspan="3">
                                    <div style="height: 163px;width: 800px; float: left;" class="img-box full">
                                        <section class="img-section" data-imgnum="4">
                                            <input id="bigImgUrl" name="prizeImgAry" type="hidden" class='filevalue' value="${entity.introductionUrl}"/>
                                            <div class="z_photo upimg-div clear" style="overflow: hidden;height: auto;" id="photoId">
                                                <section class="z_file fl">
                                                    <img src="<%=cpath %>/inc/vpoints/img/a11.png" class="add-img">
                                                    <input type="file" name="file" id="file" class="file" value=""
                            ...
                            <tr style="height: 163px;">
                                ...
                                <td class="ab_main" colspan="3">
                                    <div style="height: 163px;width: 800px; float: left;" class="img-box full">
                                        <section class="img-section" data-imgnum="4">
                                            <input id="threeImgUrl" name="prizeImgAry" type="hidden" class='filevalue' value="${entity.posterBackgroundUrl}"/>
                                            <div class="z_photo upimg-div clear" style="overflow: hidden;height: auto;" id="photoId">
                                                <section class="z_file fl">
                                                    <img src="<%=cpath %>/inc/vpoints/img/a11.png" class="add-img">
                                                    <input type="file" name="file" id="file" class="file" value=""
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="summernote">${entity.rule}</div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="remarks" class="form-control" autocomplete="off" maxlength="50" value="${entity.remarks}" />
                                    </div>
                                </td>
                            </tr>
                    ...
                    <div class="widget-header top-only">
                        <h4><i class="iconfont icon-xinxi"></i>被邀请人扫码达标活动区域</h4>
                    </div>
                    <div class="widget-content panel no-padding filteruser">
                        <table class="active_board_table">
                            <tr id="shareAreaTr">
                                <td class="ab_left" style="display:table-cell; vertical-align:middle;"><label class="title">区域选择：<span class="required">*</span></label></td>
                                <td class="ab_main" colspan="3">
                        ...
                        <table class="active_board_table">
                            ...
                            <tr>
                                ...
                                <td class="ab_main sku" colspan="3">
                                    ...
                                    <c:forEach items="${entity.skuKeyArray}" var="skuKey" varStatus="idx">
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" name="newUserType" value="0" style="float: left;  margin-top: 12px; cursor: pointer;"  <c:if test="${entity.newUserType eq '0'}">checked</c:if> > <span class="blocker en-larger" style="margin-left: 2px;" /> 当年未在本省区扫码</span>
                                        <input type="radio" name="newUserType" value="1" style="float: left;  margin-top: 12px; cursor: pointer;"  <c:if test="${entity.newUserType eq '1'}">checked</c:if> ><span class="blocker en-larger" style="margin-left: 2px;" /> 累计未在本省区扫码</span>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td class="ab_main" colspan="6">
                                    <table id="newUserPrize" class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10" style="width: 97%; margin: 0 auto; text-align: center; overflow: auto !important">
                                        ...
                                        <tbody>
                                            <c:forEach items="${fn:split(entity.newUserRule,';')}" var="item" varStatus="idx">
                                                <tr>
                                                    <%--阶梯--%>
                                                    <td>
                                                        <label id="NO" style="line-height: 30px;">${fn:split(item,',')[0]}</label>
                                                        <input id="ladder" name="ladder" value="${fn:split(item,',')[0]}" style="display: none" />
                                                    </td>
                                                    <%--达标人数--%>
                                                    <td>
                                                        <input type="text" id="numOfPeople" name="numOfPeople" class="form-control input-width-small number integer maxValue"  autocomplete="off"
                                                               tag="validate" data-oldval="0" maxVal="999999" maxlength="6" style="display: initial; width: 132px !important;"
                                                               value="${fn:split(item,',')[1]}" />
                                                    </td>
                                                    <%--激励类型--%>
                                                    <td>
                                                        <select id="rewardType" name="rewardType" class="form-control input-width-small" style="display: initial; width: 60px !important;">
                                                            <option value="0" <c:if test="${fn:split(item,',')[2] eq '0'}">selected</c:if>>现金</option>
                                                            <option value="1" <c:if test="${fn:split(item,',')[2] eq '1'}">selected</c:if>>积分</option>
                                                        </select>
                                                    </td>
                                                    <%--激励范围--%>
                                                    <td>
                                                        <div id="moneyCog" class="random-prize content" <c:if test="${fn:split(item,',')[2] eq '1'}">style="display: none"</c:if>>
                                                            <input type="text" name="minMoney" class="form-control input-width-small money maxValue"  autocomplete="off"
                                                                   tag="validate" data-oldval="0.00" maxVal="999.99" maxlength="6" style="position: relative;left: 9%;width: 30% !important"
                                                                   value="${fn:split(item,',')[3]}" />
                                                            ...
                                                            <input type="text" name="maxMoney" class="form-control input-width-small money maxValue"  autocomplete="off"
                                                                   tag="validate" data-oldval="0.00" maxVal="999.99" maxlength="6" style="position: relative;left: 23%;width: 30% !important"
                                                                   value="${fn:split(item,',')[4]}" />
                                                        </div>
                                                        <div id="vpointsCog" class="random-prize content" <c:if test="${fn:split(item,',')[2] eq '0'}">style="display: none"</c:if>>
                                                            <input type="text" name="minVpoints" class="form-control input-width-small number maxValue"
                                                                   autocomplete="off" tag="validate" data-oldval="0" maxVal="999" maxlength="3" style="position: relative;left: 9%;width: 30% !important"
                                                                   value="${fn:split(item,',')[5]}" />
                                                            <label style="position: relative;right: 35%;top: 3px">&nbsp;-&nbsp;</label>
                                                            <input type="text" name="maxVpoints" class="form-control input-width-small number maxValue"
                                                                   autocomplete="off" tag="validate" data-oldval="0" maxVal="999" maxlength="3" style="position: relative;left: 23%;width: 30% !important"
                                                                   value="${fn:split(item,',')[6]}" />
                                                        </div>
                                                    </td>
                                                    <%--操作--%>
                                                    <td>
                                                        <label id="addPrizeItem" class="btn-txt-add-red"><c:choose><c:when test="${idx.index eq 0}">新增</c:when><c:otherwise>删除</c:otherwise></c:choose></label>
                                                    </td>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
                            ...
                            <tr>
                                <td class="ab_main" colspan="3">
                                    <div style="margin-top: 10px; color: #db2d2a; font-size: 12px; text-align: center;">
                                        备注：达标次数范围最大值（示例：第一阶梯配置2，则表示1-2，第二阶梯配置4，表示3-4，下同）
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input id="oldUserIsInvolved" name="oldUserIsInvolved" style="float: left; margin-top:9px" type="checkbox" value="1" <c:if test="${entity.oldUserIsInvolved eq '1'}">checked</c:if>/>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <%--老用户奖励设置--%>
                            <c:choose>
                                <c:when test="${entity.oldUserIsInvolved eq '1'}">
                                    ...
                                    <tr class="reward-setting" >
                                        <td class="ab_main" colspan="6">
                                            <table id="oldUserPrize" class="table table-checkable table-striped table-hover table-bordered plus_end plus_top dt_table mart10" style="width: 97%; margin: 0 auto; text-align: center; overflow: auto !important">
                                                ...
                                                <tbody>
                                                <c:forEach items="${fn:split(entity.oldUserRule,';')}" var="item" varStatus="idx">
                                                    <tr>
                                                            <%--阶梯--%>
                                                        <td>
                                                            <label id="NO" style="line-height: 30px;">${fn:split(item,',')[0]}</label>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <div id="vpointsCog" class="random-prize content" style="display: none">
                                                            <input type="text" name="minVpoints" class="form-control input-width-small number maxValue"
                                                                   autocomplete="off" tag="validate" data-oldval="0" value="0" maxVal="999" maxlength="3" style="position: relative;left: 9%;width: 30% !important" />
                                                            <label style="position: relative;right: 35%;top: 3px">&nbsp;-&nbsp;</label>
                                                            <input type="text" name="maxVpoints" class="form-control input-width-small number maxValue"
                                                                   autocomplete="off" tag="validate" data-oldval="0" value="0" maxVal="999" maxlength="3" style="position: relative;left: 23%;width: 30% !important" />
                                                        </div>
                                                    </td>
                                                        <%--操作--%>
                                                    <td>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="relationDays" type="number" class="form-control input-width-larger integer minValue maxValue required"
                                               autocomplete="off" maxlength="5" minval="1" maxVal="365" tag="validate"
                                               value="${entity.relationDays}"/>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="incentiveNum" type="number" class="form-control input-width-larger integer minValue maxValue required"
                                               autocomplete="off" maxlength="4" minval="0" maxVal="9999" tag="validate"
                                               value="${entity.incentiveNum}"/>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="maxIncentiveNum" type="number" class="form-control input-width-larger integer minValue maxValue required"
                                               autocomplete="off" value="${entity.maxIncentiveNum}" maxlength="6" minval="0" maxVal="999999" tag="validate" />
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="maxIncentiveNumDay" type="number" class="form-control input-width-larger integer minValue maxValue required"
                                               autocomplete="off" value="${entity.maxIncentiveNumDay}" maxlength="6" minval="0" maxVal="999999" tag="validate" />
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="maxIncentiveForInviter" type="number" class="form-control input-width-larger integer minValue maxValue required"
                                               autocomplete="off" value="${entity.maxIncentiveForInviter}" maxlength="6" minval="0" maxVal="999999" tag="validate" />
                                        <span class="blocker en-larger"> 次奖励（0表示不限制）</span>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                    ...
                    <%--费用限制--%>
                    <div class="widget-header top-only">
                        <h4><i class="iconfont icon-xinxi"></i>费用限制</h4>
                    </div>
                    <div class="widget-content panel no-padding filteruser">
                        <table class="active_board_table">
                            <%--活动总投放金额--%>
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="money"  type="number" class="form-control input-width-larger  number money preValue minValue maxValue required"
                                               autocomplete="off" minval="0" maxVal="999999999.99" maxlength="12" tag="validate"
                                               value='<fmt:formatNumber value="${entity.money}" pattern="0.00"></fmt:formatNumber>' />
                            ...
                            <%--活动总投放积分--%>
                            <tr>
                                <td class="ab_left" ><label class="title">活动总投放积分：<span class="required">*</span></label></td>
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="vpoints" type="number" class="form-control input-width-larger integer minValue maxValue required"
                                               autocomplete="off" maxlength="9" minval="0" maxVal="999999999" tag="validate"
                                               value='<fmt:formatNumber value="${entity.vpoints}" pattern="0"></fmt:formatNumber>'/>
                                        <span class="blocker en-larger"> 积分（0表示总投放积分为0积分，最大积分为999,999,999积分）</span>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <div class="active_table_submit mart20">
                        <div class="button_place">
                            ...
                            <button class="btn btnReturn btn-radius3" data-url="<%=cpath%>/invitationWithGifts/showActivityList.do?vjfSessionId=${vjfSessionId}&isBegin=1">返 回</button>
                        </div>
                    </div>
                </div>
