    ...
    <script>
        $(function(){
            ...
            $("#addItem").click(function(){
                var url = "<%=cpath%>/invitationWithGifts/showAddActivity.do";
            ...
            $("a.edit").on("click", function(){
                ...
                var url = "<%=cpath%>/invitationWithGifts/showEditActivity.do?infoKey="+key;
                $("form").attr("action", url);
                $("form").submit();
            });
            ...
            $("a.del").on("click", function(){
                var key = $(this).parents("td").data("key");
                var url = "<%=cpath%>/invitationWithGifts/deleteActivity.do?infoKey="+key;
            ...
            $("a.code").on("click", function(){
                var key = $(this).parents("td").data("key");
                var url = "<%=cpath%>/invitationOrder/showOrderLst.do?activityKey="+key;
                $("form").attr("action", url);
                $("form").submit();
            });
        });
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-12 tabbable">
                <a href="<%=cpath%>/invitationWithGifts/showActivityList.do?vjfSessionId=${vjfSessionId}&isBegin=0"
                   class="btn <c:if test="${isBegin eq '0'}">btn-blue</c:if>">待上线</a>
                <a href="<%=cpath%>/invitationWithGifts/showActivityList.do?vjfSessionId=${vjfSessionId}&isBegin=1"
                   class="btn <c:if test="${isBegin eq '1'}">btn-blue</c:if>">已上线</a>
                <a href="<%=cpath%>/invitationWithGifts/showActivityList.do?vjfSessionId=${vjfSessionId}&isBegin=2"
                   class="btn <c:if test="${isBegin eq '2'}">btn-blue</c:if>">已下线</a>
                <a href="<%=cpath%>/invitationWithGifts/showActivityList.do?vjfSessionId=${vjfSessionId}&isBegin=3"
            ...
            <div class="col-md-12 tabbable tabbable-custom">
                <div class="widget box">
                    ...
                    <div class="widget-content form-inline">
                        <form class="listForm" method="post" action="<%=cpath%>/invitationWithGifts/showActivityList.do">
                                        ...
                                        <c:when test="${fn:length(resultList) gt 0}">
                                            <c:forEach items="${resultList}" var="activity" varStatus="idx">
                                                <tr>
                                                    ...
                                                    <c:if test="${isBegin eq '1'}">
                                                        <td style="text-align:center;">
                                                            <span class="thousandth">${activity.vpoints}</span>
                                                        </td>
                                                    </c:if>
                                                    <c:if test="${isBegin eq '1'}">
                                                        <td style="text-align:center;">
                                                            <span class="thousandth">${activity.putInVpoints}</span>
                                                        </td>
                                                    </c:if>
                                                    <td style="text-align:center;">
                                                    ...
                                                    <td data-key="${activity.infoKey}" style="text-align: center;">
                                                        ...
                                                        <c:if test="${activity.isBegin eq '0'}">
                                                            ...
                                                            <a class="btn btn-xs code btn-orange"><i class="iconfont icon-xiugai"></i>&nbsp;邀请码</a>
