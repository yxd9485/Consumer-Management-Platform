  ...
  <head>
	...
	<script>
		...
		$(function(){
         ...
         // 初始化筛选区域
			var filterAreaCodeAry = "${terminalInfo.areaCode}".split(",");
		...
		function initPage() {
			...
			var imgData='${terminalInfo.imageUrl}';
			if(imgData){
				showImg(imgData);
			}
  ...
  <body>
    ...
    <div class="crumbs">
        <ul id="breadcrumbs" class="breadcrumb">
            ...
            <li class="current"><a title=""> 编辑门店</a></li>
        </ul>
    </div>
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
            	action="<%=cpath %>/terminal/updateTerminal.do">
                <input type="hidden" name="queryParam" value="${queryParam}" />
                <input type="hidden" name="pageParam" value="${pageParam}" />
            	<input type="hidden" name="terminalKey" value="${terminalInfo.terminalKey}" />
            	<input type="hidden" name="imageUrl" value="${terminalInfo.imageUrl}" />
            	<input type="hidden" name="longitude" value="${terminalInfo.longitude}" />
                <input type="hidden" name="latitude" value="${terminalInfo.latitude}" />
            	<div class="widget box">
            		<div class="widget-header">
            			<h4><i class="iconfont icon-xinxi"></i>门店信息</h4>
            		</div>
                	<div class="widget-content panel no-padding">
                		<table class="active_board_table">

                			<input type="hidden" name="oldPrizeTyleLst" value="${terminalInfo.oldPrizeTyleLst}" />
							<tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="terminalName" tag="validate"  
	                       					class="form-control input-width-larger required" autocomplete="off" maxlength="30" value="${terminalInfo.terminalName}" />
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="address" tag="validate"  
	                       					class="form-control input-width-larger required varlength" autocomplete="off" data-length="80" maxlength="80" value="${terminalInfo.address}" />
	                       				<label class="validate_tips"></label>
	                       			</div>
	                       		</td>
	                       	</tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="coordinate" tag="validate"  
	                       					class="form-control input-width-larger required varlength" autocomplete="off" data-length="50" maxlength="50" value="${terminalInfo.coordinate}" />
	                       				<div class="content">
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
	                       				<input name="phoneNum" tag="validate"  
	                       					class="form-control input-width-larger required varlength" autocomplete="off" data-length="15" maxlength="15"  value="${terminalInfo.phoneNum}" />
	                       				<label class="validate_tips"></label>
	                       			</div>
	                       		</td>
	                       	</tr>
	                       	...
	                       	<td class="ab_main" colspan="3">
	                       	 ...
	                       	 <tr>
	                       		...
	                       		<td class="ab_main activity">
	                       			...
	                       			<c:forEach items="${fn:split(terminalInfo.oldPrizeTyleLst, ',')}" var="prizeKey" varStatus="idx">
		                       			<div class="content activity" style="margin: 5px 0px; display: flex;">
		                       				<select class="form-control input-width-larger" id="prizeKyes" name="prizeKyes">
	                       						...
	                       						<c:forEach items="${bigPrizes }" var="item">
	                       							<option value="${item.prizeNo }" 
	                       								<c:if test="${prizeKey eq item.prizeNo}"> selected="selected" </c:if>>
	                       								${item.prizeName }
	                       							</option>
	                       						</c:forEach>
		                       				</select>
