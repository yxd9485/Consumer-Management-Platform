	...
	<script>
		$(function(){
        	...
        	$("select[name='activityType']").on("change",function(){
        		if($(this).val() == '1'){
	    			$("input[name='exchangeVpoints']").attr("disabled", "disabled").val("");
	    			$("input[name='openingNumber']").removeAttr("disabled");
	    			$("input[name='virtualNumber']").removeAttr("disabled");
	    			//$("select[name='isDedicated']").removeAttr("disabled");
	    			$("select[name='isDedicated']").trigger("change");
	    		}else if($(this).val() == '2'){
	    			$("input[name='exchangeVpoints']").removeAttr("disabled");
	    			$("input[name='openingNumber']").attr("disabled", "disabled").val("");
	    			$("input[name='virtualNumber']").attr("disabled", "disabled").val("");
            ...
            $("select[name='activityType']").trigger("change");
		});
		...
		// 检验月擂台是否重复
		function checkMonthBidding(infoKey, startDate, endDate){
			$.ajax({
		...
		function validForm() {
			...
			// 检验名称是否重复
			/* var infoKey = $("input[name='activityKey']").val();
			var activityName = $("input[name='activityName']").val();
        	...
        	if(activityType == '2'){
            	...
            	// 检验月擂台是否重复
            	checkMonthBidding(infoKey, startDate, endDate);
            	if(!flagStatus){
            		return false;
            	}
  ...
  <body>
    ...
    <div class="crumbs">
        <ul id="breadcrumbs" class="breadcrumb">
        	...
        	<li class="current"><a title="">修改擂台</a></li>
        </ul>
    </div>
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
            	action="<%=cpath %>/biddingActivity/doBiddingActivityEdit.do">
                		...
                		<table class="active_board_table">
	                       	<tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="activityName" value="${biddingActivity.activityName }"
                			...
                			<tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content">
                                        <input name="startDate" id="startDate" class="form-control input-width-medium Wdate required preTime" value="${biddingActivity.startDate }"
                                            tag="validate" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', maxDate:'#F{$dp.$D(\'endDate\')}'})"  />
                                        <span class="blocker en-larger">至</span>
                                        <input name="endDate" id="endDate" class="form-control input-width-medium Wdate required sufTime"  value="${biddingActivity.endDate }"
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content" style="display: flex; margin: 5px 0px;">
                                        <select name="activityType" class="form-control input-width-larger required" tag="validate" disabled="disabled">
                                        	<option value="">请选择</option>
                                            <option value="1" <c:if test="${biddingActivity.activityType eq '1' }"> selected </c:if>>日擂台
                                            <option value="2" <c:if test="${biddingActivity.activityType eq '2' }"> selected </c:if>>月擂台</option>
                                        </select>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            ...
                            <tr id="dedicatedTr">
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content" style="display: flex; margin: 5px 0px;">
                                        <select name="isDedicated" class="form-control input-width-larger required" tag="validate"  disabled="disabled">
                                            <option value="0">否</option>
                                            <option value="1" <c:if test="${biddingActivity.isDedicated eq '1' }"> selected </c:if>>是</option>
                                        </select>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                                    ...
                                    <div class="content" style="display: flex; margin: 5px 0px;">
                                        <select name="goodsId" class="form-control input-width-larger required" tag="validate">
	                                    	...
	                                    	<c:forEach items="${goodsList }" var="item">
	                                    		<option value="${item.goodsId}" <c:if test="${biddingActivity.goodsId eq item.goodsId }"> selected </c:if>>${item.goodsName }</option>
	                                    	</c:forEach>
                                        </select>
                                        <label class="validate_tips"></label>
                                    </div>
                            ...
                            <tr id="openingNumberTr">
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="openingNumber"  value="${biddingActivity.openingNumber }"
                            ...
                            <tr id="virtualNumberTr">
                                <td class="ab_left"><label id="everyonePeriodLimit" class="title"><span class="required">*</span>虚拟开场人数：</label></td>
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="virtualNumber"  value="${biddingActivity.virtualNumber }"
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" class="tab-radio" name="prizeExpireType" value="0" style="float:left;" 
                                        	<c:if test="${biddingActivity.prizeExpireType eq '0' }"> checked="checked" </c:if>/>
                                        	
                                        <span class="blocker en-larger" style="margin-left: 2px;">指定日期</span>
                                        <input name="prizeExpireDate" class="form-control input-width-medium Wdate required"  value="${biddingActivity.prizeExpireDate }"
                                            tag="validate" autocomplete="off" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:'%y-%M-%d'})" 
                                            <c:if test="${biddingActivity.prizeExpireType eq '1' }"> disabled="disabled" </c:if>/>
                                            
                                        <input type="radio" class="tab-radio" name="prizeExpireType" value="1" style="float:left; margin-left: 10px !important;" 
                                        	<c:if test="${biddingActivity.prizeExpireType eq '1' }"> checked="checked" </c:if>/>
                                        	
                                        <span class="blocker en-larger" style="margin-left: 2px;">有效天数</span>
                                        <input name="prizeValidDay" tag="validate" value="${biddingActivity.prizeValidDay }" 
                                        	<c:if test="${biddingActivity.prizeExpireType eq '0' }"> disabled="disabled" </c:if>
                            ...
                            <tr id="vpointsTr">
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="exchangeVpoints"  value="${biddingActivity.exchangeVpoints }"
