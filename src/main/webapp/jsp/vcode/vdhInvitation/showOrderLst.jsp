    ...
    <script>
        $(function(){
            ...
            $("a.shop").on("click", function(){
                var url = "<%=cpath%>/vdhInvitationCode/showCodeLst.do?vjfSessionId=${vjfSessionId}&tabsFlag=1";
                window.location.href = url;
            });

            // 创建订单
            $("#addItem").click(function(){
                $("#orderWindow input[name='orderName']").val("");
                $("#orderWindow input[name='orderNum']").val(1);
            ...
            $("a.edit").on("click", function(){
                ...
                $("#orderWindow input[name='orderNum']").val($(this).closest("tr").find("#orderNum").text());
                $("#orderWindow tr.reward-setting").hide();
                $("#orderWindow").modal("show");
            });
            ...
            $("#orderWindow").delegate("#addBtn", "click", function(){
                ...
                if (infoKey == ""){
                    ...
                    $.fn.confirm(tips, function(){
                        // 提交表单
                        var url = "<%=cpath%>/vdhInvitationOrder/addOrder.do";
                        var paramJson = {};
                    ...
                    $.fn.confirm(tips, function(){
                        // 提交表单
                        var url = "<%=cpath%>/vdhInvitationOrder/editOrder.do";
                        var paramJson = {};
                        $("#orderWindow :input[name]").each(function(){
                            paramJson[$(this).attr("name")] = $(this).val();
                        });
            ...
            $("a.del").on("click", function(){
                ...
                var url = "<%=cpath%>/vdhInvitationOrder/deleteOrder.do?infoKey="+key;
            ...
            $("a.addCode").click(function(){
                ...
                var key = $td.data("key");
                var url = "<%=cpath%>/vdhInvitationOrder/createCode.do?infoKey="+key+"&projectServerName=${projectServerName}";
        ...
        function checkName(infoKey){
            ...
            $.ajax({
                url : "${basePath}/vdhInvitationOrder/checkName.do",
                data:{
                    "infoKey":infoKey,
                    "checkName":orderName
                },
        ...
        function downloadQrcode(orderKey, projectServerName) {
            var url = "${basePath}/vdhInvitationOrder/exportCode.do?orderKey=" + orderKey
...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-12 tabbable">
            <a href="<%=cpath%>/vdhInvitationOrder/showOrderLst.do?vjfSessionId=${vjfSessionId}&tabsFlag=0"
               class="btn <c:if test="${tabsFlag eq '0'}">btn-blue</c:if>">二维码订单</a>
            <a class="btn <c:if test="${tabsFlag eq '1'}">btn-blue</c:if> shop">门店码</a>
        </div>
    </div>
        ...
        <div class="col-md-12 tabbable tabbable-custom">
            <div class="widget box">
                ...
                <div class="widget-content form-inline">
                    <form class="listForm" method="post" action="<%=cpath%>/vdhInvitationOrder/showOrderLst.do">
                                    ...
                                    <c:when test="${fn:length(resultList) gt 0}">
                                        <c:forEach items="${resultList}" var="item" varStatus="idx">
                                            <tr>
                                                ...
                                                <td style="text-align:center;">
                                                    <span id="orderNum">${item.orderNum}</span>
                                                </td>
                                                <td style="text-align:center;">
                                                    <span>
                                                        <c:if test="${item.status eq '0'}">未生成</c:if>
                                                        <c:if test="${item.status eq '1'}">正在生成</c:if>
                                                        <c:if test="${item.status eq '2'}">生成成功</c:if>
                                                        <c:if test="${item.status eq '3'}">生成失败</c:if>
                                                    </span>
                                                </td>
                                                <td style="text-align:center;">
                                                ...
                                                <td data-key="${item.infoKey}" style="text-align: center;">
                                                    <c:if test="${item.status eq '0'}">
                                                        ...
                                                        <a class="btn btn-xs addCode btn-orange"><i class="iconfont icon-xiugai"></i>&nbsp;生成码源</a>
                                                    </c:if>
                                                    ...
                                                    <c:if test="${item.status eq '2'}">
                                                        <a class="btn btn-xs export btn-orange"
                                                           onclick="downloadQrcode('${item.infoKey}', '${projectServerName}')"><i class="iconfont icon-xiugai"></i>&nbsp;下载二维码</a>
                                                    </c:if>
                                                    <c:if test="${item.status eq '3'}">
                                                        <a class="btn btn-xs edit btn-orange"><i class="iconfont icon-xiugai"></i>&nbsp;修改</a>
                                                        <a class="btn btn-xs del btn-orange"><i class="iconfont icon-xiugai"></i>&nbsp;删除</a>
                                                        <a class="btn btn-xs addCode btn-orange"><i class="iconfont icon-xiugai"></i>&nbsp;重新生成</a>
                                                    </c:if>
                                                </td>
                                            </tr>
                                        </c:forEach>
                    ...
                    <table class="active_board_table" style="line-height: 30px;">
                        ...
                        <tr>
                            ...
                            <td class="ab_main" colspan="3">
                                <div class="content">
                                    <input name="orderNum" value="1" type="number" class="form-control input-width-larger integer minValue maxValue required"
