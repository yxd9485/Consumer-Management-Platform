...
<head>
    ...
    <script>
        ...
        function initPage() {
            ...
            // 初始化大区配置
            var regionArr = '${entity.bigRegion}'.split(",");
            ...
            var regionLstSize = '${regionLstSize}';
            if (regionArr.length ==  regionLstSize){
    ...
    <div class="row">
        <div class="col-md-12 tabbable tabbable-custom">
            <form method="post" class="form-horizontal row-border validate_form" id="code_form"
                  action="<%=cpath %>/vdhInvitation/editActivity.do">
                ...
                <div class="widget box">
                                ...
                                <td class="ab_main sku" colspan="3">
                                    ...
                                    <c:forEach items="${entity.skuKeyArray}" var="skuKey" varStatus="idx">
                                        <div class="content sku promotionSku" style="display: flex; margin: 5px 0px;">
                                            ...
                                            <label class="title mart5 btn-txt-add-red" style="font-weight: normal; margin-left: 5px; color:green" id="addSku">${idx.count == 1 ? '新增' : '删除'}</label>
                                            <label class="validate_tips"></label>
                                        </div>
                                    </c:forEach>
                                        ...
                                        <tbody>
                                            <c:forEach items="${fn:split(entity.newUserRule,';')}" var="item" varStatus="idx">
                                                <tr>
                                                    <%--阶梯--%>
                                                    <td>
                                                        <label name="NO" style="line-height: 30px;">${fn:split(item,',')[0]}</label>
                                                        <input name="ladder" value="${fn:split(item,',')[0]}" style="display: none" />
                                                    </td>
                                                    <%--达标人数--%>
                                                    <td>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="maxIncentiveForInviterDay" type="number" class="form-control input-width-larger integer minValue maxValue required"
                                               autocomplete="off" value="${entity.maxIncentiveForInviterDay}" maxlength="6" minval="0" maxVal="999999" tag="validate" />
                                        <span class="blocker en-larger"> 次（0表示不限制）</span>
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <div class="widget-content panel no-padding filteruser">
                        <table class="active_board_table">
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" name="inviterAreaType" value="0" style="float: left;  margin-top: 12px; cursor: pointer;"  <c:if test="${entity.inviterAreaType eq '0'}">checked</c:if> /> <span class="blocker en-larger" style="margin-left: 2px;"> 组织架构</span>
                                        <input type="radio" name="inviterAreaType" value="1" style="float: left;  margin-top: 12px; cursor: pointer;"  <c:if test="${entity.inviterAreaType eq '1'}">checked</c:if> /><span class="blocker en-larger" style="margin-left: 2px;"> 终端分组</span>
                                    </div>
                                </td>
                            </tr>

                            <%--活动区域--%>
                            <tr class="region" <c:if test="${entity.inviterAreaType eq '1'}">style="display: none;"</c:if>>
                            ...
                            <tr class="group" <c:if test="${entity.inviterAreaType eq '0'}">style="display: none;"</c:if>>
                                <td class="ab_left"><label class="title">活动区域：<span class="required">*</span></label>
                                </td>
                                <td class="ab_main terminalGroupTdCog" colspan="3">
                                    <c:choose>
                                        <c:when test="${entity.inviterAreaType eq '1'}">
                                            <c:forEach items="${entity.terminalGroupArray}" var="terminalGroup" varStatus="idx">
                                                <div class="content" style="display: flex; margin: 5px 0px;">
                                                    <select class="form-control input-width-larger required terminalGroup" name="tGroup"
                                                        ...
                                                        <c:forEach items="${groupLst}" var="item">
                                                            <option value="${item.terminalGroup}" <c:if test="${terminalGroup == item.terminalGroup}"> selected</c:if> >${item.terminalGroup}</option>
                                                        </c:forEach>
                                                    </select>
                                                    <label class="title mart5 btn-txt-add-red addTerminalGroup"
                                                           style="font-weight: normal; margin-left: 5px;" id="addTerminalGroup">${idx.count == 1 ? '新增' : '删除'}</label>
                                                    <label class="validate_tips"></label>
                                                </div>
                                            </c:forEach>
                                        </c:when>
                        ...
                        <table class="active_board_table">
                            ...
                            <tr style="height: 163px;">
                                ...
                                <td class="ab_main" colspan="3">
                                    <div style="height: 163px;width: 800px; float: left;" class="img-box full">
                                        <section class="img-section" data-imgnum="4">
                                            <input id="fourImgUrl" name="prizeImgAry" type="hidden" class='filevalue' value="${entity.modalWindowUrl}"/>
                                            <div class="z_photo upimg-div clear" style="overflow: hidden;height: auto;" id="photoId">
                                                <section class="z_file fl">
                                                    <img src="<%=cpath %>/inc/vpoints/img/a11.png" class="add-img">
                                                    <input type="file" name="file" id="file" class="file" value=""
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input type="radio" name="routineRebateSwitch" value="1" style="float: left;  margin-top: 12px; cursor: pointer;"  <c:if test="${entity.routineRebateSwitch eq '1'}">checked</c:if>  /> <span class="blocker en-larger" style="margin-left: 2px;"> 是</span>
                                        <input type="radio" name="routineRebateSwitch" value="0" style="float: left;  margin-top: 12px; cursor: pointer;"  <c:if test="${entity.routineRebateSwitch eq '0'}">checked</c:if>  /><span class="blocker en-larger" style="margin-left: 2px;"> 否</span>
                                    </div>
                                </td>
                            </tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="prompt" tag="validate" class="form-control input-width-larger required"
                                               autocomplete="off" maxlength="50" value="${entity.prompt}" />
                                        <label class="validate_tips"></label>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <div class="active_table_submit mart20">
                        <div class="button_place">
                            ...
                            <button class="btn btnReturn btn-radius3" data-url="<%=cpath%>/vdhInvitation/showActivityList.do?vjfSessionId=${vjfSessionId}&isBegin=1">返 回</button>
                        </div>
                    </div>
                </div>
