            	...
            	<div class="widget box">
                		...
                		<table class="active_board_table">
                			<tr>
	                       	...
	                       	<tr>
                        		...
                        		<td class="ab_main date">
                        			<c:forEach items="${fn:split(morescanCog.validDateRange,',')}" var="validDateRange" varStatus="idx">
	                        			<div class="content date" style="margin: 5px 0px;  display: flex;">
	                                        <input name="beginDate" id="beginDate${idx.index}" class="Wdate form-control input-width-medium required preTime"  autocomplete="off"
	                                            tag="validate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', maxDate:'#F{$dp.$D(\'endDate${idx.index}\')}'})" value="${fn:split(validDateRange,'#')[0]}"/>
	                                        ...
	                                        <input name="endDate" id="endDate${idx.index}" class="Wdate form-control input-width-medium required sufTime" value="${fn:split(validDateRange,'#')[1]}" 
	                                            tag="validate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:'#F{$dp.$D(\'beginDate${idx.index}\')}'})" autocomplete="off"/>
                			...
                			<tr>
                        		...
                        		<td class="ab_main time">
                        			<c:forEach items="${fn:split(morescanCog.validTimeRange,',')}" var="validTimeRange" varStatus="idx">
	                        			<div class="content time" style="margin: 5px 0px; display: flex;">
	                                        ...
	                                        tag="validate" onfocus="WdatePicker({dateFmt:'HH:mm:ss', maxDate:'#F{$dp.$D(\'endTime${idx.index}\')}'})" value="${fn:split(validTimeRange,'#')[0]}"/>
	                                        <span class="blocker en-larger">至</span>
	                                        <input name="endTime" id="endTime${idx.index}" class="form-control input-width-medium required Wdate"  autocomplete="off"
	                                        tag="validate" onfocus="WdatePicker({dateFmt:'HH:mm:ss', minDate:'#F{$dp.$D(\'beginTime${idx.index}\')}'})" value="${fn:split(validTimeRange,'#')[1]}"/>
                            ...
                            <tr>
                                <td class="ab_left"><label class="title mart5">指定热区：<span class="required">&nbsp;</span></label></td>
                                <td class="ab_main">
                                        ...
                                        <select name="hotAreaKey" class="hotArea form-control" style="width: 480px;">
                                            ...
                                            <c:if test="${not empty(hotAreaList) }">
                                                <c:forEach items="${hotAreaList}" var="hotArea">
                                                    <option value="${hotArea.hotAreaKey }" <c:if test="${hotArea.hotAreaKey == morescanCog.hotAreaKey }">selected="selected"</c:if>>${hotArea.hotAreaName}</option>   
                                                </c:forEach>
                                            </c:if>
                                        </select>
                                        <label class="validate_tips"></label>
                			...
                			<tr>
	                       		<td class="ab_left"><label class="title">中出模式：<span class="white">*</span></label></td>
	                       		<td class="ab_main" colspan="3">
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="validCount" tag="validate" class="form-control required number positive" autocomplete="off" maxlength="1" style="width: 240px;" value= "${morescanCog.validCount}"/>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
                                        <input name="qrcodeValidHour" tag="validate" class="form-control required number minValue" autocomplete="off" maxlength="6" minVal="0" style="width: 240px;" value= "${morescanCog.qrcodeValidHour}"/>
									...
									<td class="ab_main" colspan="3">
										<div class="content scanType required">
											<div style="float: left; margin-right: 10px;">
												<span>按次数中奖</span>
												<input type="radio" tag="validate" class="required" <c:if test="${morescanCog.roleInfo == null}"> checked </c:if> name="scanType" value="0" style="float: left;">
											</div>
											<div style="float: left; margin-right: 10px;">
												<span>按角色中奖</span>
												<input type="radio" tag="validate" class="required"  <c:if test="${morescanCog.roleInfo != null}"> checked </c:if> name="scanType" value="1" style="float: left;">
											</div>
											<label class="validate_tips"></label>
										</div>
									</td>
								</tr>
								<tr id="roleInfoTr" <c:if test="${morescanCog.roleInfo == null}"> hidden </c:if>>
											...
											<c:if test="${not empty(roleInfoAll)}">
												<c:forEach items="${roleInfoAll }" var="roleItem">
													<div style="float: left; margin-right: 10px;">
														...
														<c:forEach items="${fn:split(morescanCog.roleInfo, ',') }" var="role">
															<c:if test="${fn:split(roleItem, ':')[0] eq role}"> checked="checked" </c:if>
														</c:forEach>
                            ...
                            <tr>
	                       		...
	                       		<td class="ab_main" colspan="3">
	                       			<div class="content firstRebate">
	                       			    <input type="hidden" name="vcodeActivityKey" value="${morescanCog.vcodeActivityKey}"/>
	                       				<select class="form-control input-width-larger" disabled="disabled" id="vcodeActivityKey" name="vcodeActivityKey">
	                       					<option value="">请选择</option>
	                       					<c:forEach items="${activityList }" var="activity">
	                       						<option value="${activity.vcodeActivityKey }@${activity.skuName }@${activity.skuType }" 
	                       							<c:if test="${morescanCog.vcodeActivityKey eq activity.vcodeActivityKey}">selected</c:if>>
                            ...
                            <tr>
                                ...
                                <td class="ab_main" colspan="3">
                                    <div class="content">
									    ...
									    data-off-color="warning" <c:if test="${morescanCog.status eq '1'}"> checked </c:if>/>
                                    </div>
                                </td>
                            </tr>
                		</table>
                    ...
                    <div class="active_table_submit mart20">
                        <div class="button_place">
                        	<c:if test="${morescanCog.isBegin ne '2'}">
