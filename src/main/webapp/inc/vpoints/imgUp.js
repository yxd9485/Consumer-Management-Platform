...
$(function(){
	...
	function changeDo(){
		...
		if(fileList.length > 4 || totalNum > 4 ){
			...
			var src=jQuery.parseJSON(uploadImg(file));
			$(".z_file").append("<input type='file' name='file' id='file' class='file' value='' accept='image/jpg,image/jpeg,image/png,image/bmp,image/gif' multiple />");
			$(".file").change(changeDo);
			if(src["errMsg"]=="error"){
				alert("上传失败");
				return;
...
//						});
				...
				var $img = $("<img class='up-img up-opcity' style='width:auto; margin:auto;' onerror='imgLoadErr(this)' >");
...
function imgLoadErr(upImg) {
	setTimeout(function(){
		$(upImg).attr("src", $(upImg).attr("src"));
	}, 1500);
}
function showImg(src){
	var srcList=src.split(",");
	var imgContainer=$("#photoId");
	for(var i = 0;i<srcList.length;i++){
		var imgUrl = srcList[i];
		imgSrc.push(imgUrl);
		imgArr.push(imgUrl);
