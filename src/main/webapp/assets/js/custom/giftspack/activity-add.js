$(function(){
	...
	$("select[name='vcodeActivityKey']").change(function(){
		...
		if(start && end && start != "" && end != ""){
			...
			//此时放开红包奖品的添加、删除、关联占比
			$("a[flag='prizeAdd'], a[flag='prizeDel'], a[flag='prizeRelate']").removeClass("disabled").addClass("active");
		}else{
			//此时关闭红包奖品的添加、删除、关联占比
			$("a[flag='prizeAdd'], a[flag='prizeDel'], a[flag='prizeRelate']").removeClass("active").addClass("disabled");
		}
		//返利活动改变,则删除所有的奖品规则
		//$(".prize-edit").empty();
		//清空积分红包成本
	...
	$("a[flag='prizeAdd']").click(function(){
		var prizeHtml = '<tr>'
						...
						+'<td class="awards-count-td" style="width:6%;"><input autocomplete="off" name="ruleList[0].awardsCount" class="awards-count rule-count form-control" type="text" maxlength="11" /></td>'
	...
	$(".btn-save, .btn-publish").click(function(){
		...
		for(var i=0; i < $trs.length; i++){
			...
			if($.trim(awardsName.val()) == ""){
				alert("奖品名称不能为空"); awardsName.focus(); return false;
			}else{
				//判断奖品名称是否有重复
	...
	//				2、实物奖与积分奖有有效期，积分奖与券没有有效期，控制有效期输入框
	...
	//奖品概率失去焦点，计算奖品数量
	$(".prize-edit").delegate(".awards-percentage", "blur", function(){
		if(!$(this).prop("readonly")){
			var percentage = $(this).val();
			if(percentage != ""){
				var tr = $(this).parents("tr:first");
				var count = $("select[name='vcodeActivityKey']").find("option:selected").attr("counts");
				if(count && count != "" && count > 0){
					count = unThousandth($.trim(count));
					var newCount = count * percentage / 100;
					tr.find(".awards-count").val(thousandth(parseInt(newCount) + "")).blur();
					//重新计算总积分红包成本
					vjfCount();
				};
			};
