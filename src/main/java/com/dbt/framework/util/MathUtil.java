...
public class MathUtil {
	 ...
	 * @param value 将被转换的字符串
	 * @param length 精确到小数点后的位数
	 * @param bl true 返回带","逗号的字符串; false 返回不带","逗号的字符串
	 * @return 转换后得到的double型数字
	 */
	...
	public static String convertMoneyToChinese(double money) {
		...
		if (!x13.equals(" ")) {
			...
			if ((chinese.length() > 0) && !chinese.equals("负") && x13.equals("0") && !x14.equals("0")) {
				chinese = chinese + "零";
	 ...
	 * @return
	 */
	public static String maxNumber(String mx) {
		int maxInt = Integer.parseInt(mx) + 1;
		int zeroCount = String.valueOf(mx).length() - String.valueOf(maxInt).length();
		StringBuffer stringBuffer = new StringBuffer(8);
		for (int i = 0; i < zeroCount; i++) {
			stringBuffer.append("0");
		}
		return stringBuffer.toString() + maxInt;
	}
	public static void main(String[] args) {
		...
		System.out.println("四舍五入[" + a + "]保留1位小数：" + String.valueOf(round(a, 1)));
		System.out.println("解析字符串[" + c + "]：" + String.valueOf(parse(c)));
		System.out.println("大写转换[" + c + "]：" + String.valueOf(convertMoneyToChinese(parse(c))));
		
		double point = Double.valueOf(3);
		point = point / 100;
		DecimalFormat format = new DecimalFormat("0.00");
		System.out.println(format.format(point));
