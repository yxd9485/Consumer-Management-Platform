 ...
 * 未生成成功的自动码源订单Job
 */
@Service("vpsQrcodeAutoOrderJob")
public class VpsQrcodeAutoOrderJob {
    
    @Autowired
    private VpsQrcodeAutoOrderService qrcodeAutoOrderService;
    
    /**
     * 查询未生成成功的自动码源订单，重新发起生成操作
     */
    @SuppressWarnings("unchecked")
    public void dealUnCreateOrder() throws Exception {
        // 获取job执行的省区
        DbContextHolder.clearDBType();
        Set<String> nameList = null;
        String projectServerNames = DatadicUtil.getDataDicValue(
                DatadicKey.dataDicCategory.PROJECT_JOB,
                DatadicKey.ProjectJob.QRCODE_AUTO_ORDER_FAILED);
        ...
        for (String projectServerName : nameList) {	
            ...
            qrcodeAutoOrderService.dealUnCreateOrderJob();
            DbContextHolder.clearDBType();
            Thread.sleep(500);
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
	public void batchBindActivity() throws Exception {
		// 获取job执行的省区
		DbContextHolder.clearDBType();
    	Set<String> nameList = null;
    	String projectServerNames = DatadicUtil.getDataDicValue(
    	...
    	for (String projectServerName : nameList) {	
			DbContextHolder.setDBType(projectServerName);
			qrcodeAutoOrderService.executeAutoQrcodeQrderBatchBindActivity();
			DbContextHolder.clearDBType();
			Thread.sleep(500);
		}
    }
