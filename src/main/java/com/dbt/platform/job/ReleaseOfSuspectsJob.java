...
@Service("releaseOfSuspectsJob")
public class ReleaseOfSuspectsJob {
    @Autowired
    VcodeBlacklistService vcodeBlacklistService;
	...
	public void  updateReleaseOfSuspects() throws Exception{
		...
		DbContextHolder.clearDBType();
	   	...
	   	String projectServerNames = DatadicUtil.getDataDicValue(
				...
				DatadicKey.ProjectJob.UPDATE_RELEASE_OF_SUSPECTS);
		...
		if(!"ALL".equals(projectServerNames)){
	   	...
	   	for (String projectServerName : nameList) {	
			...
			vcodeBlacklistService.updateReleaseOfSuspects();
			DbContextHolder.clearDBType();
			Thread.sleep(500);
		 }
	}
